...
static const char *cmd2str(u8 cmd)
{
	...
	case WIFI_CMD_GET_INFO:
		return "WIFI_CMD_GET_INFO";
	case WIFI_CMD_SET_REGDOM:
		return "WIFI_CMD_SET_REGDOM";
	case WIFI_CMD_OPEN:
		return "WIFI_CMD_OPEN";
	case WIFI_CMD_CLOSE:
		return "WIFI_CMD_CLOSE";
	case WIFI_CMD_POWER_SAVE:
		return "WIFI_CMD_POWER_SAVE";
	case WIFI_CMD_SET_PARAM:
		return "WIFI_CMD_SET_PARAM";
	case WIFI_CMD_REQ_LTE_CONCUR:
		return "WIFI_CMD_REQ_LTE_CONCUR";
	case WIFI_CMD_SYNC_VERSION:
		return "WIFI_CMD_SYNC_VERSION";
	case WIFI_CMD_CONNECT:
		return "WIFI_CMD_CONNECT";

	case WIFI_CMD_SCAN:
		return "WIFI_CMD_SCAN";
	case WIFI_CMD_SCHED_SCAN:
		return "WIFI_CMD_SCHED_SCAN";
	case WIFI_CMD_DISCONNECT:
		return "WIFI_CMD_DISCONNECT";
	case WIFI_CMD_KEY:
		return "WIFI_CMD_KEY";
	case WIFI_CMD_SET_PMKSA:
		return "WIFI_CMD_SET_PMKSA";
	case WIFI_CMD_GET_STATION:
		return "WIFI_CMD_GET_STATION";
	case WIFI_CMD_SET_CHANNEL:
		return "WIFI_CMD_SET_CHANNEL";

	case WIFI_CMD_START_AP:
		return "WIFI_CMD_START_AP";
	case WIFI_CMD_DEL_STATION:
		return "WIFI_CMD_DEL_STATION";
	case WIFI_CMD_SET_BLACKLIST:
		return "WIFI_CMD_SET_BLACKLIST";
	case WIFI_CMD_SET_WHITELIST:
		return "WIFI_CMD_SET_WHITELIST";
	case WIFI_CMD_MULTICAST_FILTER:
		return "WIFI_CMD_MULTICAST_FILTER";

	case WIFI_CMD_TX_MGMT:
		return "WIFI_CMD_TX_MGMT";
	case WIFI_CMD_REGISTER_FRAME:
		return "WIFI_CMD_REGISTER_FRAME";
	case WIFI_CMD_REMAIN_CHAN:
		return "WIFI_CMD_REMAIN_CHAN";
	case WIFI_CMD_CANCEL_REMAIN_CHAN:
		return "WIFI_CMD_CANCEL_REMAIN_CHAN";

	case WIFI_CMD_SET_IE:
		return "WIFI_CMD_SET_IE";
	case WIFI_CMD_NOTIFY_IP_ACQUIRED:
		return "WIFI_CMD_NOTIFY_IP_ACQUIRED";

	case WIFI_CMD_SET_CQM:
		return "WIFI_CMD_SET_CQM";
	case WIFI_CMD_SET_ROAM_OFFLOAD:
		return "WIFI_CMD_SET_ROAM_OFFLOAD";
	case WIFI_CMD_SET_MEASUREMENT:
		return "WIFI_CMD_SET_MEASUREMENT";
	case WIFI_CMD_SET_QOS_MAP:
		return "WIFI_CMD_SET_QOS_MAP";
	case WIFI_CMD_TDLS:
		return "WIFI_CMD_TDLS";
	case WIFI_CMD_11V:
		return "WIFI_CMD_11V";
	case WIFI_CMD_NPI_MSG:
		return "WIFI_CMD_NPI_MSG";
	case WIFI_CMD_NPI_GET:
		return "WIFI_CMD_NPI_GET";

	case WIFI_CMD_ASSERT:
		return "WIFI_CMD_ASSERT";
	case WIFI_CMD_FLUSH_SDIO:
		return "WIFI_CMD_FLUSH_SDIO";
	case WIFI_CMD_ADD_TX_TS:
		return "WIFI_CMD_ADD_TX_TS";
	case WIFI_CMD_DEL_TX_TS:
		return "WIFI_CMD_DEL_TX_TS";
	case WIFI_CMD_LLSTAT:
		return "WIFI_CMD_LLSTAT";

	case WIFI_CMD_GSCAN:
		return "WIFI_CMD_GSCAN";
	case WIFI_CMD_RTT:
		return "WIFI_CMD_RTT";
	case WIFI_CMD_RSSI_MONITOR:
		return "WIFI_CMD_RSSI_MONITOR";

	case WIFI_CMD_IBSS_JOIN:
		return "WIFI_CMD_IBSS_JOIN";
	case WIFI_CMD_SET_IBSS_ATTR:
		return "WIFI_CMD_SET_IBSS_ATTR";
	case WIFI_CMD_NAN:
		return "WIFI_CMD_NAN";
	case WIFI_CMD_RND_MAC:
		return "WIFI_CMD_RND_MAC";
	case WIFI_CMD_BA:
		return "WIFI_CMD_BA";
	case WIFI_CMD_SET_MAX_CLIENTS_ALLOWED:
		return "WIFI_CMD_SET_MAX_CLIENTS_ALLOWED";
	case WIFI_CMD_TX_DATA:
		return "WIFI_CMD_TX_DATA";
	case WIFI_CMD_ADDBA_REQ:
		return "WIFI_CMD_ADDBA_REQ";
	case WIFI_CMD_DELBA_REQ:
		return "WIFI_CMD_DELBA_REQ";
	case WIFI_CMD_SET_PROTECT_MODE:
		return "WIFI_CMD_SET_PROTECT_MODE";
	case WIFI_CMD_GET_PROTECT_MODE:
		return "WIFI_CMD_GET_PROTECT_MODE";
	case WIFI_CMD_DOWNLOAD_INI:
		return "WIFI_CMD_DOWNLOAD_INI";
	case WIFI_CMD_PACKET_OFFLOAD:
		return "WIFI_CMD_PACKET_OFFLOAD";
#ifdef DFS_MASTER
	case WIFI_CMD_RADAR_DETECT:
		return "WIFI_CMD_RADAR_DETECT";
	case WIFI_CMD_RESET_BEACON:
		return "WIFI_CMD_RESET_BEACON";
#endif
	case WIFI_CMD_HANG_RECEIVED:
		return "WIFI_CMD_HANG_RECEIVED";
	case WIFI_CMD_VOWIFI_DATA_PROTECT:
		return "WIFI_CMD_VOWIFI_DATA_PROTECT";
	case WIFI_CMD_SET_TLV:
		return "WIFI_CMD_SET_TLV";
	case WIFI_CMD_SET_SAE_PARAM:
		return "WIFI_CMD_SET_SAE_PARAM";
	case WIFI_CMD_EXTENDED_LLSTAT:
		return "WIFI_CMD_EXTENDED_LLSTAT";
	default:
			 ...
			 unsigned int timeout, u8 *rbuf, u16 *rlen)
{
	...
	if (ret != -1) {
		if (unlikely(intf->cp_asserted) || unlikely(intf->exit)) {
			wl_err("%s go to err!\n", __func__);
			ret = -1;
			goto err;
		}
...
void sprdwl_event_scan_done(struct sprdwl_vif *vif, u8 *data, u16 len)
{
	...
	case SPRDWL_SCAN_ABORT_DONE:
		...
		netdev_info(vif->ndev, "%s scan abort got %d BSSes\n",
			    __func__, bss_count);
		break;
	case SPRDWL_SCAN_ASSERT:
		if ((time == 0) || ((jiffies - time) >=
			msecs_to_jiffies(30*60*1000))) {
			netdev_info(vif->ndev, "%s scan assert\n", __func__);
			mdbg_assert_interface("scan assert!");
		}
		time = jiffies;
		break;
...
static const char *evt2str(u8 evt)
{
	...
	case WIFI_EVENT_CONNECT:
		return "WIFI_EVENT_CONNECT";
	case WIFI_EVENT_DISCONNECT:
		return "WIFI_EVENT_DISCONNECT";
	case WIFI_EVENT_SCAN_DONE:
		return "WIFI_EVENT_SCAN_DONE";
	case WIFI_EVENT_MGMT_FRAME:
		return "WIFI_EVENT_MGMT_FRAME";
	case WIFI_EVENT_MGMT_TX_STATUS:
		return "WIFI_EVENT_MGMT_TX_STATUS";
	case WIFI_EVENT_REMAIN_CHAN_EXPIRED:
		return "WIFI_EVENT_REMAIN_CHAN_EXPIRED";
	case WIFI_EVENT_MIC_FAIL:
		return "WIFI_EVENT_MIC_FAIL";
	case WIFI_EVENT_NEW_STATION:
		return "WIFI_EVENT_NEW_STATION";
	case WIFI_EVENT_CQM:
		return "WIFI_EVENT_CQM";
	case WIFI_EVENT_MEASUREMENT:
		return "WIFI_EVENT_MEASUREMENT";
	case WIFI_EVENT_TDLS:
		return "WIFI_EVENT_TDLS";
	case WIFI_EVENT_SDIO_SEQ_NUM:
		return "WIFI_EVENT_SDIO_SEQ_NUM";
	case WIFI_EVENT_SDIO_FLOWCON:
		return "WIFI_EVENT_SDIO_FLOWCON";
	case WIFI_EVENT_BA:
		return "WIFI_EVENT_BA";
	case WIFI_EVENT_RSSI_MONITOR:
		return "WIFI_EVENT_RSSI_MONITOR";
	case WIFI_EVENT_GSCAN_FRAME:
		return "WIFI_EVENT_GSCAN_FRAME";
	case WIFI_EVENT_RADAR_DETECTED:
		return "WIFI_EVENT_RADAR_DETECTED";
	case WIFI_EVENT_STA_LUT_INDEX:
		return "WIFI_EVENT_STA_LUT_INDEX";
	case WIFI_EVENT_SUSPEND_RESUME:
		return "WIFI_EVENT_SUSPEND_RESUME";
	case WIFI_EVENT_NAN:
		return "WIFI_EVENT_NAN";
	case WIFI_EVENT_RTT:
		return "WIFI_EVENT_RTT";
	case WIFI_EVENT_HANG_RECOVERY:
		return "WIFI_EVENT_HANG_RECOVERY";
	case WIFI_EVENT_THERMAL_WARN:
		return "WIFI_EVENT_THERMAL_WARN";
	case WIFI_EVENT_WFD_MIB_CNT:
		return "WIFI_EVENT_WFD_MIB_CNT";
	case WIFI_EVENT_FW_PWR_DOWN:
		return "WIFI_EVENT_FW_PWR_DOWN";
	case WIFI_EVENT_CHAN_CHANGED:
		return "WIFI_EVENT_CHAN_CHANGED";
	case WIFI_EVENT_COEX_BT_ON_OFF:
		return "WIFI_EVENT_COEX_BT_ON_OFF";
			       ...
			       int data_len)
{
	...
	wl_info("total len is: %d, data len is: %d\n", len, data_len);
