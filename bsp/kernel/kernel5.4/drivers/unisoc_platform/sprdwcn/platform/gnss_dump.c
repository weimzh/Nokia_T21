...
unsigned int gnss_bsp_state_sync;
EXPORT_SYMBOL_GPL(gnss_bsp_state_sync);

int gnss_dump_mem(char flag)
{
	int ret = 0;

	if (gnss_bsp_state_sync == WCN_POWER_STATUS_OFF_GOING) {
		GNSSDUMP_INFO("gnss power status off ongoing:can not dump gnss\n");
		return -1;
	}
