...
void wcn_reset_process(bool force_reset)
{
		...
		struct mbuf_t *head, struct mbuf_t *tail, int num)
{
	...
	if (sprdwcn_bus_get_carddump_status() || wcn_is_assert()) {
		if (num != 1) {
			WCN_INFO("%s mbuf(%d) does not allow sending(WCN ASSERT)\n",
				__func__, num);
			return false;
		}
...
bool wcn_assert_need_reset(char *source)
{
	int i = 0;
	char * const ast_need_reset[] = {
		"scan assert!", /* Wi-Fi need reset */
	};

	WARN_ON(wcn_sysfs_get_reset_prop() == WCN_ASSERT_BOTH_RESET_DUMP);

	if (wcn_sysfs_get_reset_prop() != WCN_ASSERT_ONLY_DUMP)
		return true;

	for (i = 0; i < ARRAY_SIZE(ast_need_reset); i++) {
		if (!strncmp(source, ast_need_reset[i], strlen(ast_need_reset[i]))) {
			pr_info("%s WCN dump is converted to reset because:%s\n", __func__, source);
			return true;
		}
	}
...
void wcn_assert_interface(enum wcn_source_type type, char *str)
{
	...
	if (reset_prop == WCN_ASSERT_ONLY_DUMP && wcn_assert_need_reset(str)) {
		reset_prop = WCN_ASSERT_ONLY_RESET;
		force_reset = true;
	}

	if (!mdbg_proc->assert_notify_flag && likely(!force_reset)) {
		/* sysdump notify? */
