			...
			struct wcn_device *wcn_dev)
{
	...
	if (strcmp(wcn_dev->name, WCN_MARLIN_DEV_NAME) == 0) {
		...
		WCN_INFO("index = %d, dbus base = 0x%x, size = 0x%x\n", index,
		...
		if (ret) {
			...
			wcn_dev->dbus.dbus_reg_base = ioremap(wcn_dev->dbus.phy_reg, wcn_dev->dbus.dbus_max_offset);
		} else {
		...
		WCN_INFO("index = %d, phy_reg=0x%x,size=0x%x,map to dbus_reg_base=0x%x\n", index,
