...
EXPORT_SYMBOL(asoc_sprd_card_set_ops);
	...
	struct snd_soc_dai_link *dai_link, struct sprd_card_data *priv)
{
	...
	if (ret) {
		...
		name = devm_kzalloc(dev,
			...
			strlen(dai_link->codecs->dai_name) + 2, GFP_KERNEL);
		if (name)
	...
	if (ret) {
		...
		stream_name = devm_kzalloc(dev,
			...
			strlen(dai_link->codecs->dai_name) + 2, GFP_KERNEL);
		if (stream_name)
			sprintf(stream_name, "%s-%s", dai_link->cpus->dai_name,
				dai_link->codecs->dai_name);
		else {
			stream_name = NULL;
					  ...
					  struct snd_soc_dai_link_component *codec)
{
	...
	if (val[1] == 1) {	/* dummy playback-only codec */
		codec->dai_name = "sprd-dummy-playback-dai";
		codec->name = "sprd-dummy-playback";
	} else {
				      ...
				      int idx, bool is_top_level_node)
{
	...
	struct snd_soc_dai_link_component *component = NULL;
	...
	/* Get room for cpu/codec/platform of snd_soc_dai_link,
	      but only malloc one struction space for every component temporarily. */
	component = devm_kzalloc(dev,
				       sizeof(struct snd_soc_dai_link_component) * 3,
				       GFP_KERNEL);
	dai_link->cpus = component;
	dai_link->codecs = component + 1;
	...
	/* -- awinic add for register multi codec -- start */
	pr_info("plat parse info : awinic codec_name = %s codec_dai_name = %s cpu_dai_name = %s multi_codecs = %d\n",
			dai_link->codecs->name, dai_link->codecs->dai_name, dai_link->cpus->dai_name, multi_codecs);
	...
	if (ret < 0) {
		...
		pr_info("%s: snd_soc_of_get_dai_link_codecs failed. Go to use a dummy codec.\n",
		     __func__);
		ret = asoc_sprd_card_dummy_codec_sel(codec, dai_link->codecs);
		if (ret)
			goto dai_link_of_err1;
	...
	for (i = 0; i < dai_link->num_codecs; i++) {
		pr_info("Asoc codecs info: codec[%d], codec_name = %s codec_dai_name = %s\n",
			i, dai_link->codecs[i].name, dai_link->codecs[i].dai_name);	
	}

	/* -- awinic add for register multi codec -- end */
	...
	if (ret) {
		pr_info("%s: asoc_sprd_card_dai_link_of_misc return error.\n",
			     __func__);
		goto dai_link_of_err1;
	}
	...
	/* -- awinic add for register multi codec -- start */
	/* clear old parse info */
	/*if (dai_link->num_codecs > 1 && multi_codecs) {
		pr_info("Asoc use codecs bind, clear codec_dai_name and codec_of_node\n");
...
static int asoc_sprd_card_unref(struct snd_soc_card *card)
{
	...
	for (num_links = 0, dai_link = card->dai_link;
		...
		if (dai_link->cpus != NULL && dai_link->num_cpus > 0) {
			of_node_put(dai_link->cpus->of_node);
			of_node_put(dai_link->platforms->of_node);
...
EXPORT_SYMBOL(asoc_sprd_card_probe);
...
EXPORT_SYMBOL(asoc_sprd_card_remove);
