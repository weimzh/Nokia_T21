...
#define AWINIC_CALI_FILE  "/mnt/vendor/audio/aw_cali.bin"
...
static ssize_t aw_re_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	int ret, i=0;
	struct aw883xx *aw883xx = dev_get_drvdata(dev);
	struct aw_device *aw_dev = aw883xx->aw_pa;
	...
	if (is_single_cali) {
		...
		if (ret <= 0) {
			...
			for (i = 0; i < ret; i++)
			{
				len += snprintf(buf+len, PAGE_SIZE-len, "%u",re[i]);
				if(i == 0)
				len += snprintf(buf+len, PAGE_SIZE-len, ",");
			}
			len += snprintf(buf+len, PAGE_SIZE-len, " \n");
...
static ssize_t aw_f0_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	int ret, i=0;
	struct aw883xx *aw883xx = dev_get_drvdata(dev);
	struct aw_device *aw_dev = aw883xx->aw_pa;
	...
	if (is_single_cali) {
		...
		if (ret <= 0) {
			...
			for (i = 0; i < ret; i++)
			{
				len += snprintf(buf+len, PAGE_SIZE-len, "%u",f0[i]);
				if(i == 0)
				len += snprintf(buf+len, PAGE_SIZE-len, ",");
			}
			len += snprintf(buf+len, PAGE_SIZE-len, " \n");
