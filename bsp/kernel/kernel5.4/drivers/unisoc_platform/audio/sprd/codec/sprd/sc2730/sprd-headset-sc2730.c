 ...
 */
static int dapm_jack_switch_control(struct snd_soc_component *codec, bool on)
{
	struct snd_kcontrol *kctrl;
	struct snd_soc_card *card = codec ? codec->card : NULL;
...
static void sprd_headset_gain_init(void)
{
    headset_reg_write(ANA_CDC7, HPL_G(0x5),
               HPL_G(0xf));
    headset_reg_write(ANA_CDC7, HPR_G(0x5),
               HPR_G(0xf));
    pr_info("%s set headset gain to -3db\n", __func__);
}
...
int sprd_headset_soc_probe(struct snd_soc_component *codec)
{
	...
	hdst->hdst_detect_wakelock = wakeup_source_create("headset_detect_wakelock");
	wakeup_source_add(hdst->hdst_detect_wakelock);
...
failed_to_request_int_all_irq:
	...
	wakeup_source_remove(hdst->hdst_detect_wakelock);
	wakeup_source_destroy(hdst->hdst_detect_wakelock);
