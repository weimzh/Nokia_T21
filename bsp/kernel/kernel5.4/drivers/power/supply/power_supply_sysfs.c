...
static struct power_supply_attr power_supply_attrs[] = {
	...
	//add by jack,20220514, start/stop charge attr
	POWER_SUPPLY_ATTR(USB_CHARGE_TYPE),
	POWER_SUPPLY_ATTR(CAP_SWITCH),
//	POWER_SUPPLY_ATTR(CAPACITY_MAX_CHARGE),
#endif
};
...
int power_supply_uevent(struct device *dev, struct kobj_uevent_env *env)
{
	...
	for (j = 0; j < psy->desc->num_properties; j++) {
		...
		if (psy->desc->properties[j] == POWER_SUPPLY_PROP_STOPCHARGING_TEST
				|| psy->desc->properties[j] == POWER_SUPPLY_PROP_STARTCHARGING_TEST || psy->desc->properties[j] == POWER_SUPPLY_PROP_USB_CHARGE_TYPE || psy->desc->properties[j] == POWER_SUPPLY_PROP_CAP_SWITCH) {
			continue;
		}
		//add by jack,20220514, start/stop charge attr
		#endif
		if (psy->desc->properties[j] == POWER_SUPPLY_PROP_MANUFACTURE_MONTH || psy->desc->properties[j] == POWER_SUPPLY_PROP_MANUFACTURE_DAY) {
			continue;
		}
