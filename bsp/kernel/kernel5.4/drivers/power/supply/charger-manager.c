...
#define CM_FAST_CHARGE_DISABLE_CAP       900
#define CM_FAST_CHARGE_DISABLE_CAP_HIGH       500
#define CM_FAST_CHARGE_DISABLE_CAP_LOW       750

static int cm_fast_charge_disable_cap = 900;
...
static int fast_9V = 0;
static int fuel_cap_custom = 0;
static int full_cap_custom = 1;

static int capacity_max = 100 ;
static bool capacity_max_enable = 0;
extern bool is_resume_from_hall;
static bool map_enable = 0;
...
static bool is_charging(struct charger_manager *cm)
{
	...
	for (i = 0; cm->desc->psy_charger_stat[i]; i++) {
		...
		if (!cm->charger_enabled)
			continue;
		//add by jack, modify, 20220514, start/stop charge attr
...
static bool cm_primary_charger_enable(struct charger_manager *cm, bool enable)
{
	...
	if(fuel_cap_custom < cm_fast_charge_disable_cap){
		cm_set_charger_ovp(cm, CM_FAST_CHARGE_OVP_DISABLE_CMD);
		cm_enable_second_charger(cm, enable);
	}
	return true;
}
		...
		if (uV >= desc->fullbatt_uV && uA <= desc->fullbatt_uA && uA >= 0) {
			if (++desc->trigger_cnt > 1) {
				if (cm->desc->cap >= CM_CAP_FULL_PERCENT) {
					...
					is_full = false;
					cap_adjust = cm->desc->cap + 5;
					if(cap_adjust >=CM_CAP_FULL_PERCENT)
						cap_adjust = CM_FORCE_SET_FUEL_CAP_FULL;
					adjust_fuel_cap(cm, cap_adjust);
					//if (desc->trigger_cnt == 2)
					//	cm_primary_charger_enable(cm, false);
				}
			} else {
...
{
	int batt_ocv, batt_uA, fchg_ocv_threshold, thm_cur,batt_uV,ret;
	...
	if ((target_cur >= CM_FAST_CHARGE_ENABLE_CURRENT &&
	    ...
	    batt_ocv < fchg_ocv_threshold)||(batt_uV >= CM_FAST_CHARGE_DISABLE_BATTERY_VOLTAGE && fuel_cap_custom < cm_fast_charge_disable_cap))
		return true;
	else if ((batt_ocv >= CM_FAST_CHARGE_START_VOLTAGE_LTHRESHOLD &&
		 batt_uA >= CM_FAST_CHARGE_ENABLE_CURRENT) || (batt_uV >= CM_FAST_CHARGE_DISABLE_BATTERY_VOLTAGE && fuel_cap_custom < cm_fast_charge_disable_cap))
		return true;
...
static bool cm_is_disable_fixed_fchg_check(struct charger_manager *cm)
{
	...
	if (batt_uV < CM_FAST_CHARGE_DISABLE_BATTERY_VOLTAGE ||
	   // batt_uA < CM_FAST_CHARGE_DISABLE_CURRENT)
	      fuel_cap_custom >= cm_fast_charge_disable_cap)
...
static void cm_cp_state_exit(struct charger_manager *cm)
{
	...
	//add by jack, 20220514, start/stop charge attr
	if (!cm->charging_status && !cm->emergency_stop & !cm->force_stop_charge) {
...
/*	if (enable == cm->charger_enabled)
	...
	if (enable) {
		...
		//add by jack, 20220514, start/stop charge attr
		if (cm->force_stop_charge) {
			dev_info(cm->dev, "StopCharging_Test had been set!\n");
			return -EAGAIN;
		}
		//add by jack, 20220514, start/stop charge attr
...
#ifndef CONFIG_WT_COMPILE_NO_THERMAL_VERSION
	...
	if (cm->charger_enabled) {
		...
		if(!cm->desc->force_set_full){
			if (duration > desc->discharging_max_duration_ms) {
				...
				if(!full_cap_custom){
					dev_info(cm->dev, "recharging duration exceed %ums\n",
						desc->discharging_max_duration_ms);
					cm->charging_status &= ~CM_CHARGE_DURATION_ABNORMAL;
				}
			}
...
//#ifdef WT_COMPILE_FACTORY_VERSION
		err = power_supply_set_property(psy, POWER_SUPPLY_PROP_MANUFACTURE_YEAR, &val);
		power_supply_put(psy);
		if (err)
			return err;
//#endif
...
static int cm_manager_get_jeita_status(struct charger_manager *cm, int cur_temp)
{
	...
	/* temperature goes down */
	if (last_temp > cur_temp){
		em_temp_up_down = false;
	}
	/* temperature goes up */
	else{
		em_temp_up_down = true;
	}
...
static int cm_manager_jeita_current_monitor(struct charger_manager *cm)
{
	...
	if (desc->jeita_disabled) {
		if (last_jeita_status != cm->desc->force_jeita_status) {
			dev_err(cm->dev, "Disable jeita and force jeita state to force_jeita_status\n");
...
//add by jack,20220513,for ato charging capacity control,begin.
#ifdef CONFIG_WT_COMPILE_FACTORY_VERSION
static bool cap_switch = 1;
// add by hz 20220702 for ato no thermal version capacity control,start
#ifndef CONFIG_WT_COMPILE_NO_THERMAL_VERSION
#define ATO_HIGH_CAP 70
#define ATO_LOW_CAP 55
static bool cm_charge_check_ato(struct charger_manager *cm)
{
	int fuel_cap = -1;
	static bool ato_capacity_limited = false;

	fuel_cap = DIV_ROUND_CLOSEST(cm->desc->cap, 10);

	if(fuel_cap >= ATO_HIGH_CAP && !ato_capacity_limited){
		try_charger_enable(cm, false);
		power_supply_changed(cm->charger_psy);
		ato_capacity_limited = true;
		dev_err(cm->dev, "ATO version stop charger cap = %d\n", fuel_cap);
	//add by jack, 20220514, start/stop charge attr
	} else if(fuel_cap <= ATO_LOW_CAP && ato_capacity_limited && !cm->force_stop_charge){
		cm->charger_enabled = false;
		try_charger_enable(cm, true);
		power_supply_changed(cm->charger_psy);
		ato_capacity_limited = false;
		dev_err(cm->dev, "ATO version enable charging cap = %d\n", fuel_cap);
	}
	return ato_capacity_limited;
}
#endif
// add by hz 20220702 for ato no thermal version capacity control,end
#endif
//add by jack,20220513,for ato charging capacity control,start.
...
// add by hz 20220702 for ato no thermal version capacity control,end
	...
	get_vbat_avg_uV(cm, &vbat_avg);
	if(em_temp_up_down == true && cm->desc->temperature >= 450 && vbat_avg >= 4100000){
		printk("sqw :45~54 vbat > 4100000 temp is up,stop it\n ");
		return POWER_SUPPLY_STATUS_NOT_CHARGING;
	}
...
// add by hz 20220702 for ato no thermal version capacity control,start
	...
	get_vbat_avg_uV(cm, &vbat_avg);
	if(cm->desc->temperature >= 450 && vbat_avg >= 4100000){
		printk("sqw :45~54 vbat > 4100000 temp is up,stop it\n ");
		return POWER_SUPPLY_STATUS_NOT_CHARGING;
	}
	if (cm->force_stop_charge == true)
		return POWER_SUPPLY_STATUS_NOT_CHARGING;
#endif
// add by hz 20220702 for ato no thermal version capacity control,end
	...
	// add by hz 20220702 for ato no thermal version capacity control,start
#ifndef CONFIG_WT_COMPILE_NO_THERMAL_VERSION
	if (cm_charge_check_ato(cm)||(cm->force_stop_charge == true))
		return POWER_SUPPLY_STATUS_NOT_CHARGING;
#endif
	}
	// add by hz 20220702 for ato no thermal version capacity control,end
#endif
	//add by jack,20220513,for ato charging capacity control,end.

    if ((capacity_max_enable == 1) && (capacity_max*10 < cm->desc->cap))
		return POWER_SUPPLY_STATUS_NOT_CHARGING;
...
static bool judge_charger_type = 0;
...
static int bat_internal_resist = 0;
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STARTCHARGING_TEST:
		cm->force_stop_charge = false;
		cm->charger_enabled = false;
		try_charger_enable(cm, true);
		val->intval = 1;
		dev_err(cm->dev, "disable force_stop_charge!\n");
		ret = 0;
		break;
	case POWER_SUPPLY_PROP_STOPCHARGING_TEST:
		cm->force_stop_charge = true;
		try_charger_enable(cm, false);
		val->intval = 0;
		dev_err(cm->dev, "enable force_stop_charge!\n");
		ret = 0;
		break;
	//add by jack, 20220514, start/stop charge attr
	
	case POWER_SUPPLY_PROP_USB_CHARGE_TYPE:
		val->intval = cm->desc->charger_type;
		break;
	case POWER_SUPPLY_PROP_CAP_SWITCH:
		val->intval = cap_switch;
	break;
#endif
	case POWER_SUPPLY_PROP_MANUFACTURE_MONTH:
		val->intval = capacity_max;
	break;
	
	case POWER_SUPPLY_PROP_MANUFACTURE_DAY:
		val->intval = map_enable;
	break;
	default:
		return -EINVAL;
	}
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_MANUFACTURE_MONTH:
		capacity_max_enable = 1;
		capacity_max = val->intval;
        if(capacity_max < 30)
            capacity_max = 30;
        else if(capacity_max > 100)
            capacity_max = 100;
		break;
		
	case POWER_SUPPLY_PROP_MANUFACTURE_DAY:
		map_enable = val->intval;
		break;
		
#ifdef WT_COMPILE_FACTORY_VERSION
	case POWER_SUPPLY_PROP_CAP_SWITCH:
		cap_switch = val->intval;
		break;
#endif		
	default:
		ret = -EINVAL;
...
static enum power_supply_property default_charger_props[] = {
	...
	//add by jack,20220514, start/stop charge attr
	POWER_SUPPLY_PROP_USB_CHARGE_TYPE,
	POWER_SUPPLY_PROP_CAP_SWITCH,
	//POWER_SUPPLY_PROP_CAPACITY_MAX_CHARGE,
#endif
	/*
...
static int cm_get_bat_info(struct charger_manager *cm)
{
	...
	bat_internal_resist = cm->desc->internal_resist; 
...
static void dump_register(struct charger_manager *cm){
	struct power_supply *psy;
	int i;
//#ifdef WT_COMPILE_FACTORY_VERSION
	union power_supply_propval val;
...
//#endif
	for (i = 0; i < 2; i++) {
		psy = power_supply_get_by_name(cm->desc->psy_charger_stat[i]);
//#ifdef WT_COMPILE_FACTORY_VERSION
		ret = power_supply_get_property(psy, POWER_SUPPLY_PROP_MANUFACTURE_YEAR,
					&val);
//#endif
	}
}
static void cm_batt_works(struct work_struct *work)
{
...
#ifdef CONFIG_DISABLE_NTC
               cur_temp = 250;
               board_temp = cm->desc->temperature = cur_temp;
#endif
#ifdef CONFIG_WT_COMPILE_FACTORY_VERSION
               //cur_temp = 250;
               //board_temp = cm->desc->temperature = cur_temp;
	...
	if(board_temp >= 510 && cm->desc->charger_type == 6 && is_resume_from_hall == true)
	{	
		//printk("sqw 11111111111\n");
		temp_ibus_down = 1;
		cm->cm_charge_vote->vote(cm->cm_charge_vote, true,
				 SPRD_VOTE_TYPE_IBUS,
				 SPRD_VOTE_TYPE_IBUS_ID_FCHG_FIXED_TRANSITION,
				 SPRD_VOTE_CMD_MIN, 2100000, cm);
	}
	
	if(board_temp < 480 && cm->desc->charger_type == 6)
	{
		temp_ibus_down = 0;
		//printk("sqw 222222222222\n");
		cm->cm_charge_vote->vote(cm->cm_charge_vote, true,
				 SPRD_VOTE_TYPE_IBUS,
				 SPRD_VOTE_TYPE_IBUS_ID_FCHG_FIXED_TRANSITION,
				 SPRD_VOTE_CMD_MIN, 2200000, cm);
	}
	if(cm->desc->charger_type == 6 && is_resume_from_hall == false)
	{
		temp_ibus_down = 0;
		cm->cm_charge_vote->vote(cm->cm_charge_vote, true,
				 SPRD_VOTE_TYPE_IBUS,
				 SPRD_VOTE_TYPE_IBUS_ID_FCHG_FIXED_TRANSITION,
				 SPRD_VOTE_CMD_MIN, 2210000, cm);	
	}
	
	if(map_enable == 1 && cm->desc->charger_type == 6)
	{
		cm->cm_charge_vote->vote(cm->cm_charge_vote, true,
				 SPRD_VOTE_TYPE_IBUS,
				 SPRD_VOTE_TYPE_IBUS_ID_FCHG_FIXED_TRANSITION,
				 SPRD_VOTE_CMD_MIN, 1600000, cm);
		
	}
	if(cm->desc->charger_type == 6)
		judge_charger_type = 1;
	else
		judge_charger_type = 0;
	...
	case POWER_SUPPLY_STATUS_FULL:
		...
		if ((vbat_avg < recharge_uv) && (batt_uA < 0)) {
			cm->desc->force_set_full = false;
			full_cap_custom = 0;
	...
	fuel_cap_custom = fuel_cap;
	if(cur_temp > 450)
		cm_fast_charge_disable_cap = CM_FAST_CHARGE_DISABLE_CAP_HIGH;
	else if(cur_temp <150)
		cm_fast_charge_disable_cap = CM_FAST_CHARGE_DISABLE_CAP_LOW;
	else
		cm_fast_charge_disable_cap = CM_FAST_CHARGE_DISABLE_CAP;

	if(fuel_cap >= cm_fast_charge_disable_cap){
		cm_enable_second_charger(cm, false);
		fast_9V = 0;
		cm->cm_charge_vote->vote(cm->cm_charge_vote, true,
				 SPRD_VOTE_TYPE_IBUS,
				 SPRD_VOTE_TYPE_IBUS_ID_FCHG_FIXED_TRANSITION,
				 SPRD_VOTE_CMD_MIN, 2000000, cm);
				 
		if(cm_fast_charge_disable_cap == CM_FAST_CHARGE_DISABLE_CAP_LOW && cm->desc->charger_type == 6)
		{			
			psy = power_supply_get_by_name(cm->desc->psy_charger_stat[0]);
			val.intval = 1600000;
			power_supply_set_property(psy,POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT,&val);
		}
		if(cm_fast_charge_disable_cap == CM_FAST_CHARGE_DISABLE_CAP_HIGH && cm->desc->charger_type == 6)
		{		
			//printk("sqw high\n");
			psy = power_supply_get_by_name(cm->desc->psy_charger_stat[0]);
			val.intval = 2000000;
...
// add by hz 20220702 for ato no thermal version capacity control,start
#ifdef CONFIG_WT_COMPILE_NO_THERMAL_VERSION
	cm->desc->jeita_disabled = true;
	dev_err(&pdev->dev, "sqw: jeita_disabled: %d\n", cm->desc->jeita_disabled);
#endif
// add by hz 20220702 for ato no thermal version capacity control,end

	sysfs_create_groups(&pdev->dev.kobj, charge_groups);
