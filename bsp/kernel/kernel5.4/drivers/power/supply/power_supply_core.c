...
//add by jack for charge_type on 2022.05.12 begin
static ssize_t show_charge_type(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	batt_psy = power_supply_get_by_name("sgm41513a_charger");
	if(batt_psy) {
		rc = power_supply_get_property(batt_psy, POWER_SUPPLY_PROP_USB_TYPE, &val);
	} else {
...
//add by jack for charge_type on 2022.05.12 end
				   ...
				   bool ws)
{
	...
	struct power_supply *psy;
	int i, rc;
	int ret_device_file = 0;	//add by jack for charge_type, 2022.05.12
	...
	//Add by jack,add node : charge_type begin
	if(strcmp(psy->desc->name, "battery") == 0) {
		pr_err("battery powe supply creat charge_type attr files!!\n");
		ret_device_file = device_create_file(dev, &dev_attr_charge_type);
	}
	//Add by jack,add node : charge_type end
