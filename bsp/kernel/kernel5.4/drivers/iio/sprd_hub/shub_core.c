			    ...
			    const char *buf, size_t count)
{
	...
	dev_info(&sensor->sensor_pdev->dev,
		 "handle = %d, enabled = %d\n", handle, enabled);
	// add by hz
		if (sensor->sar.enable && (handle == SHUB_SAR0_HANDLE || handle == SHUB_SAR1_HANDLE || handle == SHUB_SAR2_HANDLE))
			sensor->sar.enable(handle, enabled);
	// add end
			   ...
			   const char *buf, size_t count)
{
	...
	// add start	
		if (sensor->sar.batch && (batch_cmd.handle == SHUB_SAR0_HANDLE || batch_cmd.handle == SHUB_SAR1_HANDLE || batch_cmd.handle == SHUB_SAR2_HANDLE))
			sensor->sar.batch();
	// add end
...
int shub_sar_register(struct sar_ctrl sar)
{
	...
	if(g_sensor->sar.batch == NULL)
		pr_err("%s sar.batch is NULL\n", __func__);
	if(g_sensor->sar.enable == NULL)
		pr_err("%s sar.enable is NULL\n", __func__);

	if(g_sensor->sar.batch)
		pr_err("%s sar.batch is true\n", __func__);
	if(g_sensor->sar.enable)
		pr_err("%s sar.enable is true\n", __func__);
