...
static bool is_dvt_hardware=false;
...
struct st21nfc_device {
	...
	/* GPIO for NFCC CLK_REQ DVT pin (input) */
	struct gpio_desc *gpiod_clkreq_dvt;
...
static bool hardware_get(void)
{
	char board_type[32];
	char* s1= "androidboot.hardware_id=";
...
static irqreturn_t st_nfc_clk_irq_thread(int irq, void *dev_id)
{
	...
	if(is_dvt_hardware) {
		enable = devm_clk_get(&st21nfc_dev->client->dev, "enable");
		value = gpiod_get_value(st21nfc_dev->gpiod_clkreq_dvt) > 0 ? true : false;
		if (value) {
			//printk("geroge nfc dvt clk enable ----1 \n");
			clk_prepare_enable(st21nfc_dev->s_clk);
			clk_prepare_enable(enable);
		} else {
			//printk("geroge nfc dvt clk disable ---0 \n");
		...
		if (value) {
			//printk("geroge nfc evt clk enable ----1 \n");
			pmic_refout_update(4, value);
		} else {
			//printk("geroge nfc evt clk disable ---0 \n");
			pmic_refout_update(4, value);
		}		
	}

	st21nfc_dev->clk_run = value;
...
static int st21nfc_clock_select(struct st21nfc_device *st21nfc_dev)
{
	...
	if(is_dvt_hardware)
		irq = gpiod_to_irq(st21nfc_dev->gpiod_clkreq_dvt);
	else
		irq = gpiod_to_irq(st21nfc_dev->gpiod_clkreq);

	ret = request_threaded_irq(irq, NULL, st_nfc_clk_irq_thread,
			IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
			st21nfc_dev->client->name, st21nfc_dev);

	if (ret < 0) {
		printk( "geroge failed to register CLK REQ IRQ handler\n");
	}
...
static int boardid_get(void)
{
	char board_type[32];
	char* s1= "androidboot.board_type=";
	struct device_node *cmdline_node = NULL;
	const char *cmdline;
	char *match, *match_end;
	int len, match_str_len, ret;
	...
	if(strcmp(board_type, "row_nfc") == 0)
		return 0;
	else
		return -1;
}
			 ...
			 const struct i2c_device_id *id)
{
    ...
    char nfc_hardware_name[HARDWARE_MAX_ITEM_LONGTH];
	...
	if(boardid_get() != 0) {
		pr_err("%s is not nfc version, exit\n", __func__);
		return -ENODEV;
	}
	pr_err("%s is nfc version\n", __func__);

	is_dvt_hardware = hardware_get();
	if(is_dvt_hardware)
		pr_err("%s is DVT/PVT/MP hardware\n", __func__);
	else
		pr_err("%s is EVT hardware\n", __func__);
...
// QCOM and MTK54 use standard GPIO definition
	...
	if(is_dvt_hardware) {
		st21nfc_dev->gpiod_clkreq_dvt = devm_gpiod_get(dev, "clkreq-dvt", GPIOD_IN);
		if (IS_ERR_OR_NULL(st21nfc_dev->gpiod_clkreq_dvt)) {
			pr_err("[OPTIONAL] %s : Unable to request clkreq-dvt-gpios\n",
				__func__);
			ret = 0;
		} else {
		...
		if (IS_ERR_OR_NULL(st21nfc_dev->gpiod_clkreq)) {
			pr_err("[OPTIONAL] %s : Unable to request clkreq-gpios\n",
				__func__);
			ret = 0;
		} else {
			//if (!device_property_read_bool(dev, "st,clk_pinctrl")) {
   ...
   /* Modify by hz for hardward infomation on 20220718 begin */
    snprintf(nfc_hardware_name, HARDWARE_MAX_ITEM_LONGTH, "ST54J");
    hardwareinfo_set_prop(HARDWARE_NFC, nfc_hardware_name);
	printk("nfc_hardware_name: %s\n", nfc_hardware_name);
    /* Modify by hz for hardward infomation on 20220718 end */
	...
	if(is_dvt_hardware) {
		...
		source = devm_clk_get(&st21nfc_dev->client->dev, "source");
		if ((source == NULL) || IS_ERR(source)) {
			return -1;
		}
		
		enable = devm_clk_get(&st21nfc_dev->client->dev, "enable");
		if ((enable == NULL) || IS_ERR(enable)) {
			return -1;
		}
		clk_set_parent(st21nfc_dev->s_clk, source);
		clk_set_rate(st21nfc_dev->s_clk, NFC_CLK_FREQ);
		...
		// add for DVT test clock always on
		#if 1
		clk_prepare_enable(st21nfc_dev->s_clk);
		clk_prepare_enable(enable);
		#endif
		...
		// add for EVT test clock always on
		#if 1		
