...
struct sprd_rtc *rtc_test;
void off_mode_charge_test(int value){
	int ret;
	u32 val;
	ret = regmap_read(rtc_test->regmap, REG_ANA_RTC_RST2, &val);
	printk("sqw %s %d:val = 0X%X\n",__func__,__LINE__,val);
	...
	ret = regmap_write(rtc_test->regmap, REG_ANA_RTC_RST1, 0);
	ret = regmap_write(rtc_test->regmap, REG_ANA_RTC_RST1, ~val);
	
	//val = 0x66;
	ret = regmap_write(rtc_test->regmap, REG_ANA_RTC_RST0, 0);
	ret = regmap_write(rtc_test->regmap, REG_ANA_RTC_RST0, val);
	
	ret = regmap_read(rtc_test->regmap, REG_ANA_RTC_RST2, &val);
	printk("sqw %s %d:val = 0X%X\n",__func__,__LINE__,val);
}
EXPORT_SYMBOL(off_mode_charge_test);
u32 off_mode_charge_value(void){
	u32 val;
	regmap_read(rtc_test->regmap, REG_ANA_RTC_RST2, &val);
	return val;
}
EXPORT_SYMBOL(off_mode_charge_value);
...
static int sprd_rtc_probe(struct platform_device *pdev)
{
	...
	rtc_test = devm_kzalloc(&pdev->dev, sizeof(*rtc), GFP_KERNEL);
	rtc_test = rtc;
