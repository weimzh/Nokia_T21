...
bool is_hall_switch = false;
EXPORT_SYMBOL(is_hall_switch);

bool is_resume_from_hall = true;
EXPORT_SYMBOL(is_resume_from_hall);

bool gesture_enable_test = 0;
EXPORT_SYMBOL(gesture_enable_test);

static bool esd_flag = 0; 
static int sprd_panel_unprepare(struct drm_panel *p)
{
	...
	DRM_INFO("%s(),gesture_enable_test=%d,esd_flag =%d\n", __func__,gesture_enable_test,esd_flag);
	is_resume_from_hall = false;
	
	if((gesture_enable_test == 0) || (esd_flag == 1)){
		...
		if (panel->info.avdd_gpio) {
  	  	...
  	  	if (panel->info.lcm1v8en_gpio) {
            gpiod_direction_output(panel->info.lcm1v8en_gpio, 0);
            mdelay(5);
    	}
...
static int sprd_panel_prepare(struct drm_panel *p)
{
	...
	if(is_hall_switch == false){
		
		is_resume_from_hall = true;

		ret = regulator_enable(panel->supply);
		if (ret < 0)
			DRM_ERROR("enable lcd regulator failed\n");
		if (panel->info.lcm1v8en_gpio) {
				gpiod_direction_output(panel->info.lcm1v8en_gpio, 1);
				mdelay(5);
		}
...
static bool powerdown_charge = 0;
static int sprd_panel_enable(struct drm_panel *p)
{
	...
	if (panel->backlight) {
		...
		panel->backlight->props.state &= ~BL_CORE_FBBLANK;
		if(powerdown_charge == 1)
			panel->backlight->props.brightness = 116;
		backlight_update_status(panel->backlight);
	}
...
static int sprd_panel_esd_check(struct sprd_panel *panel)
{
	...
	mipi_dsi_dcs_read(panel->slave, 0x45,
			  &read_val_45, 1);
			  
	mipi_dsi_set_maximum_return_packet_size(panel->slave, 4);	
	mipi_dsi_dcs_read(panel->slave, 0x09,
			  read_value, 4);
	
	mipi_dsi_set_maximum_return_packet_size(panel->slave, 3);
	mipi_dsi_dcs_read(panel->slave, 0xB1,
			  read_val_B1, 3);
	/*
	 * TODO:
...
//	if((read_val_B1[0] !=0x10) || (read_val_B1[1] !=0xFA) || (read_val_B1[2] !=0xAF))
//		B1_ERR = 1;
	
	//if ((read_val != info->esd_check_val) || (read_value[0] !=0x80) || (read_value[1] !=0x73) || (read_value[2] !=0x04) || (read_value[3] !=0x00) || (read_val_45 !=0x08) || (B1_ERR == 1)) {
	if ((read_val != info->esd_check_val) || (read_value[0] !=0x80) || (read_value[1] !=0x73) || (read_value[2] !=0x04) || (read_value[3] !=0x00) || (read_val_45 !=0x08) ) {		
		DRM_ERROR("esd check failed, read value = 0x%02x,read_value[0] =0x%02x,read_value[1] =0x%02x,read_value[2] =0x%02x,read_value[3] =0x%02x,read_val_45 = 0x%02x,read_val_B1[0] =0x%02x,read_val_B1[1] =0x%02x,read_val_B1[2] =0x%02x\n",
			  read_val,read_value[0],read_value[1],read_value[2],read_value[3],read_val_45,read_val_B1[0],read_val_B1[1],read_val_B1[2]);
		return -EINVAL;
	}
		printk("esd check OK, read value = 0x%02x,read_value[0] =0x%02x,read_value[1] =0x%02x,read_value[2] =0x%02x,read_value[3] =0x%02x,read_val_45 = 0x%02x,read_val_B1[0] =0x%02x,read_val_B1[1] =0x%02x,read_val_B1[2] =0x%02x\n",
			  read_val,read_value[0],read_value[1],read_value[2],read_value[3],read_val_45,read_val_B1[0],read_val_B1[1],read_val_B1[2]);
	return 0;
}
			...
			struct sprd_panel *panel)
{
    ...
    panel->info.lcm1v8en_gpio = devm_gpiod_get_optional(dev,
                                    "lcm1v8en", GPIOD_ASIS);
    if (IS_ERR_OR_NULL(panel->info.lcm1v8en_gpio))
            DRM_WARN("can't get panel lcm1v8en gpio: %ld\n",
                             PTR_ERR(panel->info.lcm1v8en_gpio));
...
static int sprd_panel_parse_dt(struct device_node *np, struct sprd_panel *panel)
{
	...
	if (!rc) {
		...
		efused = strstr(cmd_line, "androidboot.efused=1");
		if (efused) {
			efused_exist = 1;
			DRM_INFO("androidboot.efused=1\n");
		}else
		{
			efused_exist = 0;
			DRM_INFO("androidboot.efused=0\n");
		}			

		charge_power = strstr(cmd_line, "androidboot.mode=charger");
		if (charge_power) {
			powerdown_charge = 1;
			DRM_INFO("androidboot.mode=charger\n");
		}else
		{
			powerdown_charge = 0;
			DRM_INFO("androidboot.mode!=charger\n");
...
static const struct file_operations proc_fuse = {
    .open        = fuse_open,
    .read        = seq_read,
    .llseek      = seq_lseek,
...
static int sprd_panel_probe(struct mipi_dsi_device *slave)
{
	...
	proc_create("secboot_fuse_reg", 0666, NULL, &proc_fuse);
