...
static void sc27xx_cc_polarity_status(u32 status)
{

	if (status & SC27XX_TYPEC_FINAL_SWITCH)
		cc_status = 1;// positive:1 
	else
		cc_status = 2;// contrary:2
}
...
static irqreturn_t sc27xx_typec_interrupt(int irq, void *data)
{
	...
	ret = regmap_read(sc->regmap, sc->base + SC27XX_STATUS, &val);
		printk("SC27XX_STATUS : %d\n",val);
	if(val)
		sc27xx_cc_polarity_status(val);
	else
		cc_status = 0;   //Not insert:0
