...
#define SDHCI_SPRD_DUMP(f, x...)	pr_err("%s: " DRIVER_NAME ": " f, mmc_hostname(host->mmc), ## x)
...
/* Modify by ye_qin for mmc panic on 20220608 */
#define SDHCI_SPRD_REG_EMMC_DEBUG0	0x260
#define SDHCI_SPRD_REG_EMMC_DEBUG1	0x264
#define SDHCI_SPRD_REG_EMMC_DEBUG2	0x268
					 ...
					 unsigned int timing)
{
	...
	u16 ctrl_2;
	/* Modify by ye_qin for mmc panic on 20220608 begin */
	bool en = false;
...
{
    u32 command;
    void *desc_header = host->adma_table;
    u64 desc_ptr;
    unsigned long start = jiffies;
    struct sdhci_sprd_host *sprd_host = TO_SPRD_HOST(host);

    if (!host->mmc->card && strcmp(mmc_hostname(host->mmc), "mmc2"))
        return;
    ...
    print_hex_dump(KERN_ERR, "sprd-sdhci + 0x000: ", DUMP_PREFIX_OFFSET,
            16, 4, host->ioaddr, 64, 0);
    print_hex_dump(KERN_ERR, "sprd-sdhci + 0x200: ", DUMP_PREFIX_OFFSET,
            16, 4, host->ioaddr + 0x200, 32, 0);

    if (sprd_host->int_status & SDHCI_INT_ADMA_ERROR) {
        if (host->flags & SDHCI_USE_ADMA) {
            if (host->flags & SDHCI_USE_64_BIT_DMA) {
                desc_ptr = (u64)sdhci_sprd_readl(host,
                        SDHCI_ADMA_ADDRESS_HI) << 32;
                desc_ptr |= sdhci_sprd_readl(host, SDHCI_ADMA_ADDRESS);
            } else {
                desc_ptr = sdhci_sprd_readl(host, SDHCI_ADMA_ADDRESS);
            }

            SDHCI_SPRD_DUMP("ADMA desc_ptr: 0x%llx ERROR: 0x%08x\n",
                 desc_ptr, sdhci_sprd_readl(host, SDHCI_ADMA_ERROR));

            SDHCI_SPRD_DUMP("ADMA DEBUG0:0x%08x, DEBUG1:0x%08x DEBUG2:0x%08x\n",
                 sdhci_sprd_readl(host, SDHCI_SPRD_REG_EMMC_DEBUG0),
                 sdhci_sprd_readl(host, SDHCI_SPRD_REG_EMMC_DEBUG1),
                 sdhci_sprd_readl(host, SDHCI_SPRD_REG_EMMC_DEBUG2));

            while (desc_ptr) {
                if (host->flags & SDHCI_USE_64_BIT_DMA) {
                    struct sdhci_adma2_64_desc *dma_desc = desc_header;

                    SDHCI_SPRD_DUMP("ADMA desc 0x%p: addr: 0x%08x%08x\n",
                         desc_header,
                         le32_to_cpu(dma_desc->addr_hi),
                         le32_to_cpu(dma_desc->addr_lo));

                    SDHCI_SPRD_DUMP("ADMA len 0x%x,  attr=0x%x\n",
                         le32_to_cpu(dma_desc->len),
                         le32_to_cpu(dma_desc->cmd));

                    desc_header += host->alloc_desc_sz;
                    if (dma_desc->cmd & cpu_to_le16(ADMA2_END))
                        break;
                } else {
                    struct sdhci_adma2_32_desc *dma_desc = desc_header;

                    SDHCI_SPRD_DUMP("ADMA desc 0x%p: addr: 0x%08x\n",
                ...
                if (time_after(jiffies, start + HZ)) {
                    pr_err("ADMA error have no end desc\n");
                    break;
                }
            }
            ...
            desc_ptr |= sdhci_sprd_readl(host, SDHCI_ADMA_ADDRESS);
            SDHCI_SPRD_DUMP("Current ADMA  desc_ptr: 0x%llx\n", desc_ptr);
        }
    }
}
...
/* Modify by ye_qin for mmc panic on 20220608 end */
