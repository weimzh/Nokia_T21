...
static irqreturn_t sprd_i2c_isr_thread(int irq, void *dev_id)
{
	...
	u32 i2c_tran;
	//bool int_flag = (readl(i2c_dev->base + I2C_STATUS) & I2C_INT);
#ifdef ST_NFC_DEBUG
	printk("---enter in %s ---\n", __func__);
#endif
    if (msg->addr == 0x08 && i2c_dev->ack_flag) {
		writel(0x68, i2c_dev->base + ADDR_STA0_DVD);
		#ifdef ST_NFC_DEBUG
		 printk("---slave is STNFC---\n");
		#endif
	}
	if (msg->flags & I2C_M_RD)
...
#ifdef ST_NFC_DEBUG
	printk("---dump in %s before sprd_i2c_data_transfer---\n", __func__);
#endif
...
#ifdef ST_NFC_DEBUG
	printk("---dump in %s after sprd_i2c_data_transfer---\n", __func__);
#endif
	 ...
	 */
    #ifdef ST_NFC_DEBUG
     printk("i2c bus num: %d \n", i2c_dev->adap.nr);
    #endif
     if (!i2c_dev->ack_flag) {
        if (msg->addr == 0x08) {
            writel(0x80000, i2c_dev->base + ADDR_STA0_DVD);
            i2c_dev->err = -EIO;
            printk("---i2c-STNFC_is NACK---\n");

        } else {
            i2c_dev->err = -EIO;
            printk("---i2c-other_is NACK---\n");
        }
    }
...
#ifdef ST_NFC_DEBUG
	printk("---dump in %s after sprd_i2c_read_bytes---\n", __func__);
#endif
...
#ifdef ST_NFC_DEBUG
	printk("---dump in %s for time isr start to thread---\n", __func__);
#endif
...
static int sprd_i2c_probe(struct platform_device *pdev)
{
    ...
    i2c_dev->syscon_flag = sprd_get_i2c_syscon_reg(dev->of_node, &i2c_dev->i2c_syscon_rst, "syscons");
    if (i2c_dev->syscon_flag)
        dev_err(&pdev->dev, "failed to get syscon reg\n");
