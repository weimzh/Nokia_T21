...
static int fdt_del_reserved_mem(void *fdt, const char *c_name)
{
	...
	if (!strcmp(c_name, "cp-modem") || !strcmp(c_name, "cp-mem")) {
		sub_offset = fdt_subnode_offset(fdt, offset, "cp-modem");
		if (sub_offset == -FDT_ERR_NOTFOUND) {
			sub_offset = fdt_subnode_offset(fdt, offset, "cp-mem");
			if (sub_offset == -FDT_ERR_NOTFOUND) {
				errorf("both cp-modem and cp-mem are not exist");
				return -1;
			} else {
				ret = fdt_del_node(fdt, sub_offset);
				if (ret < 0)
					errorf("del cp-mem failed, error=%s\n", fdt_strerror(ret));
			}
		} else {
			ret = fdt_del_node(fdt, sub_offset);
			if (ret < 0)
				errorf("del cp-modem failed, error=%s\n", fdt_strerror(ret));
		}
	} else {
		sub_offset = fdt_subnode_offset(fdt, offset, "sipc-mem");
		if (offset == -FDT_ERR_NOTFOUND) {
			...
			ret = fdt_del_node(fdt, sub_offset);
			if (ret < 0)
				errorf("del %s failed, error=%s\n", c_name, fdt_strerror(ret));
		}
	}
	return 0;
}

static int fdt_del_sipc_prop(struct fdt_header *fdt, const char *s_value)
{
	int ret = 0;
	int offset;
	...
	if (offset < 0) {
		debugf("label = %s is not exist\n", s_value);
		return -1;
	} else {
	ret = fdt_delprop(fdt, offset, "label");
		if (ret < 0)
			errorf("del label = %s failed, error=%s\n", s_value, fdt_strerror(ret));
	}
	return 0;
}

static int fdt_del_node_by_label(struct fdt_header *fdt, const char *s_value)
{
	int ret = 0;
	int offset;
...
static int fdt_del_cproc_use_decoup(struct fdt_header *fdt, const char *s_name, const char *s_value)
{
	int ret = 0;
	int offset;
	int len;

	offset = fdt_node_offset_by_prop_value(fdt, 0, s_name, s_value, strlen(s_value) + 1);
	if (offset < 0) {
		debugf("%s = %s is not exist\n", s_name, s_value);
		return -1;
	} else {
		ret = fdt_del_node(fdt, offset);
		if (ret < 0)
			errorf("del %s = %s by offs failed\n", s_name, s_value);
	}
	return 0;
}
...
static int load_cp_coupling_info(void)
{
	...
	strcpy(temp, get_slot_ab(DECOUPLING_INFO_PARTITION));
	modem_part = temp;
#endif
...
#ifdef CONFIG_REMOVE_MODEM
	fdt_del_node_by_label(fdt, "stty_lte");
	fdt_del_node_by_label(fdt, "sdiag_lte");
	fdt_del_node_by_label(fdt, "spipe_lte");
	fdt_del_sipc_prop(fdt, "sipc-lte");
	fdt_del_node_by_label(fdt, "slog_lte");;
	fdt_del_node_by_label(fdt, "sipx_lte");
	fdt_del_seth_node(fdt, "sprd,seth");
	fdt_del_cproc_use_decoup(fdt, "sprd,decoup", "cproc-use-decoup");
...
int load_sp_boot_code(void)
{
	...
	printf("load sp bootcode form cproc");
        }
#ifdef SP_IRAM_ADDR
	else{
	memcpy((void *)SP_IRAM_ADDR,sp_loader_copy,sp_loader_size*sizeof(uint32_t));
	printf("load sp bootcode form modem_loader");
	}
#endif
	return 0;
}
