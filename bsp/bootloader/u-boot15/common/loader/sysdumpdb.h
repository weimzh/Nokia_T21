...
#define FULLDUMPDB_PARTITION_NAME "fulldumpdb"
...
#define MINI_STRING "mini"
#define EXTEND_NAME_MAX 20

#define BOOTLOADER_LAST_LOG	"bootloader_last_log"
#define BOOTLOADER_PRE_LOG	"bootloader_pre_log"


enum section_extend_type {
	KERNEL_SECTION = 0,	/* kernel section:text,data,bss... */
	KERNEL_OTHER_INFO,	/* kernel information, such as:vmcoreinfo, log_buf, task... */
	CP_MODEM,		/* cm4 dump*/
	UBOOT_INFO,		/* information in uboot */
	TOS_INFO,		/* tos information */
	OTHER_INFO		/* just record info self*/
};

struct section_extend_info {
	char name[EXTEND_NAME_MAX];
	uint64_t paddr;
	uint32_t size;
	uint16_t type;
};
...
struct dumpdb_header{
	...
	/*
		...
		[15 ~31]: extend bits, flow control ...

		Bit[15] : if sd dump finish , auto reboot control .  1: auto reboot ; 0: wait for key press
		Bit[16] : sizeof(unsigned long),0: 4bityes; 1: 8bytes
		Bit[17] : __attribute__(packet) struct ,0: no packet; 1: packet
		others : reserved.
	*/
	int	dump_flag;
