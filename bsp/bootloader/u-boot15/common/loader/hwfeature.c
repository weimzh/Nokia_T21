...
extern unsigned int get_soc_bonding(struct hwfeature *hwf);
...
#define HWFEATURE_SOCID_JUDGE_MODE(mode) \
    if (strcmp(phwf->key, mode) == 0) { \
                     strcpy(phwf->socinfo_value, phwf->value); \
                } \
    ...
    if (phwf->value) {
        ...
        HWFEATURE_SOCID_JUDGE_MODE("auto/efuse");
    }
...
int fdt_fixup_cpuinfo_hwfeature(void *fdt)
{
    ...
    int nodeoffset;
    int err;
    char factory[80] = "Unisoc ";
    nodeoffset = fdt_path_offset(fdt, "/");
    if (nodeoffset < 0) {
        debug("%s: Failed to find nodeoffset : %s\n", __func__, fdt_strerror(nodeoffset));
        return 0;
    }
	strcat(factory, hwf.socinfo_value);
	err = fdt_setprop(fdt, nodeoffset, "cpuinfo_hardware", factory, strlen(factory) + 1);
    if (err < 0) {
        printf("ERROR: could not setprop-cpuinfo_hardware %s.\n",fdt_strerror(err));
        return err;
    }
    return 0;
}
...
int fdt_fixup_soc_module(void *fdt)
{
    ...
    soc_module = fdt_getprop(fdt, nodeoffset, "soc-module", NULL);
    if (strcmp((const char *) soc_module, "UMS512") == 0) {
        if (get_soc_bonding(&hwf) == 0) {
            const char t618[] = "T618";
	    err = fdt_setprop(fdt, nodeoffset, "soc-module", t618, strlen(t618) + 1);
            if (err < 0) {
                printf("ERROR: could not setprop soc-module %s.\n",fdt_strerror(err));
                return err;
            }
        } else {
            const char t610[] = "T610";
            err = fdt_setprop(fdt, nodeoffset, "soc-module", t610, strlen(t610) + 1);
            if (err < 0) {
                printf("ERROR: could not setprop soc-module %s.\n",fdt_strerror(err));
                return err;
            }
        }
    } else if (strcmp((const char *) soc_module, "UMS9230") == 0) {
         if (get_soc_bonding(&hwf) == 0) {
            const char t616[] = "T616";
	    err = fdt_setprop(fdt, nodeoffset, "soc-module", t616, strlen(t616) + 1);
            if (err < 0) {
                printf("ERROR: could not setprop soc-module %s.\n",fdt_strerror(err));
                return err;
            }
        } else {
            const char t606[] = "T606";
            err = fdt_setprop(fdt, nodeoffset, "soc-module", t606, strlen(t606) + 1);
            if (err < 0) {
                printf("ERROR: could not setprop soc-module %s.\n",fdt_strerror(err));
                return err;
            }
        }
    } else if (strcmp((const char *) soc_module, "UMS9620") == 0) {
         if (get_soc_bonding(&hwf) == 0) {
              const char t760[] = "T760";
              err = fdt_setprop(fdt, nodeoffset, "soc-module", t760, strlen(t760) + 1);
              if (err < 0) {
                  printf("ERROR: could not setprop soc-module %s.\n",fdt_strerror(err));
                  return err;
              }
         } else if (get_soc_bonding(&hwf) == 1) {
              const char t770[] = "T770";
              err = fdt_setprop(fdt, nodeoffset, "soc-module", t770, strlen(t770) + 1);
              if (err < 0) {
                   printf("ERROR: could not setprop soc-module %s.\n",fdt_strerror(err));
                   return err;
              }
              ...
              const char t790[] = "T790";
              err = fdt_setprop(fdt, nodeoffset, "soc-module", t790, strlen(t790) + 1);
              if (err < 0) {
                   printf("ERROR: could not setprop soc-module %s.\n",fdt_strerror(err));
                   return err;
              }
...
int fdt_fixup_cpuinfo_hwfeature(void *fdt) { return 0; }
int fdt_fixup_soc_module(void *fdt) { return 0; }
void hwfeature_hook_late_initcall(void (*late_initcall)(struct hwfeature *phwf, void *fdt)) { };
#undef HW_GET_HOOK_MODE
#define HW_GET_HOOK_MODE(mode) \
    void hwfeature_hook_get_##mode(long (*get)(struct hwfeature *phwf)) { }
