...
int block_dev_powp(block_dev_desc_t *dev_desc, uint64_t total_part_size, uint64_t start_addr)
{
	lbaint_t start = 0;
	int wp_grp_cnt = 0;
	int wp_grp_size = 0;

	wp_grp_size = mmc_get_wp_grp_size();
	if (0 == wp_grp_size) {
		errorf("get emmc power-on write protection group size fail\n");
		return -1;
	}
	start = start_addr / dev_desc->blksz;
	wp_grp_cnt = total_part_size / (wp_grp_size*512);
	if (0 == wp_grp_cnt) {
		errorf("emmc power-on write protection group count error\n");
		return -1;
	}

	if (0 != dev_desc->block_pwr_wp(dev_desc->dev, start, wp_grp_cnt)) {
...
int common_set_powp(char *part_name)
{
	...
	if (-1 == ret || NULL == dev_desc || 0 == total_part_size)
		return -1;
	if (0 != (total_part_size % 0x800000)) {
		errorf("total partition size(0x%llx) error\n", total_part_size);
		return -1;
	}
	return block_dev_powp(dev_desc, total_part_size, part_start);
...
//20220607, Added by zhu_jun Block/unblock hardward factory reset option on begin
int check_block_factory_reset_option(char * part_name)
{
    char read_buf[64];
    if(!strcmp(part_name, "userdata")) {
        memset(read_buf, 0, sizeof(read_buf));
        if(oem_repair_read_mmc_ex("block_factory_reset", read_buf, BLOCK_FACTORY_RESET_LEN) ) {
            return 0;
        }
        return (!strncmp(read_buf, BLOCK_FACTORY_RESET_FLAG, BLOCK_FACTORY_RESET_LEN));
     } else {
        return 0;
     }
}
//20220607, Added by zhu_jun Block/unblock hardward factory reset option on end
...
//20220607, Added by zhu_jun Block/unblock hardward factory reset option on begin
    if(check_block_factory_reset_option(part_name)) {
        debugf("erase userdata should set block factory reset option 0 \n");
        return -1;
    }
//20220607, Added by zhu_jun Block/unblock hardward factory reset option on end
...
int write_file_ext4(char *mpart, char *filenm, void *buf, int len, int append)
{
#ifdef CONFIG_FS_EXT4
        static int dev_id = 0;
        char *ifname;
        ...
        if (NULL == dev_desc) {
                errorf("ext4_write_content get dev error\n");
                return -1;
        }
	...
	if (ext4fs_init() != 0) {
		errorf("error in File System init\n");
		ext4fs_deinit();
		ext4fs_close();
		return -1;
	}
	...
	if(ext4_write_file(filenm, buf, 0, len, &actwrite, append) == -1){
		errorf("** Error ext4_write_file() **\n");
		return -1;
...
int mk_dir_ext4(char *mpart, char *filenm)
{
	int ret = 0;
	static int dev_id = 0;
        char *ifname;
...
/*
  flag:
	...
	1 for minidump
*/
int format_fs_ext4(char *mpart, int flag)
{
        static int dev_id = 0;
        char *ifname;
        disk_partition_t info;
	...
	common_raw_erase(mpart, (info.size * info.blksz), 0);

        /* set the device as block device */
        ext4fs_set_blk_dev(dev_desc, &info);

	printf("Enter ext4_mkfs_fill_meta\n");
        ext4_mkfs_fill_meta(&info,flag);

	ext4fs_close();
        return 0;

}
int del_file_ext4(char *mpart, char *filenm)
{
        static int dev_id = 0;
        char *ifname;
        disk_partition_t info;
        ...
        printf("Enter ext4_del_file\n");
        ext4_del_file(filenm);
...
int rm_dir_ext4(char *mpart, char *filenm)
{
