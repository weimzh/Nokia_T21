...
int test_ufs_raw_read(int argc, char * argv[])
{
	...
	if (size != ret) {
		errorf("ufs raw read fail, size=0x%llx, offset=0x%llx!\n", size, offset);
		goto fail;
	}
...
int test_ufs_raw_write(int argc, char * argv[])
{
	...
	debugf("raw write lun(%d), offset=0x%llx, size=0x%llx\n", dev_desc->dev, offset, size);
	ret = dev_desc->block_write(dev_desc->dev, offset, size, buf);
	if (size != ret) {
		errorf("ufs raw write fail, size=0x%llx, offset=0x%llx!\n", size, offset);
		free(buf);
		return CMD_RET_FAILURE;
	}
	debugf("Raw write OK\n");
...
int test_ufs_bufread(int argc, char * argv[])
{
	...
	debugf("read part(%s), size=0x%llx, offset=0x%llx, to buffer 0x%p\n", argv[0], size, offset, buf);
	ret = common_raw_read(argv[0], size, offset, buf);
	if (-1 == ret) {
		errorf("common_raw_read fail!\n");
		return CMD_RET_FAILURE;
...
int test_ufs_bufwrite(int argc, char * argv[])
{
	...
	debugf("write part(%s), size=0x%llx, offset=0x%llx, from buffer 0x%p\n", argv[0], size, offset, buf);
	ret = common_raw_write(argv[0], size, 0, offset, buf);
	if (-1 == ret) {
		errorf("common_raw_write fail!\n");
		return CMD_RET_FAILURE;
...
int test_ufs_backwrite(int argc, char * argv[])
{
	...
	debugf("backstage write part(%s), size=0x%x, offset=0x%x, from buffer 0x%p\n", argv[0], size, offset, buf);
	...
	if (0 == strcmp(argv[0], "seq")) {
		while (1) {
			...
			while (0 != blk_todo) {
				...
				if (blk != ret) {
					errorf("ufs stress write fail, blk=0x%lx, offset=0x%lx!\n", blk, offset);
					goto fail;
				}

				ret = dev_desc->block_read(dev_desc->dev, offset, blk, buf);
				if (blk != ret) {
					errorf("ufs stress read fail, blk=0x%lx, offset=0x%lx!\n", blk, offset);
					goto fail;
				}
				blk_todo -= blk;
				offset += blk;
