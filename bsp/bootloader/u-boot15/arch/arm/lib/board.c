...
LOG_BUFFER *p_log_buffer = (LOG_BUFFER *)LOG_BUFFER_ADDR;
#else
...
extern void copy_udtb_to_dlbuf(void);
extern void choose_uboot_dtb(void);
#endif
...
static int init_log_buffer(void)
{
...
#else
	p_log_buffer->addr = LOG_BUFFER_START_ADDR;
#endif
	/* backup last */
	if (p_log_buffer->magic == 0x676f6c7544525053) {
		memcpy((uint8_t *)p_log_buffer + LOG_BUFFER_SIZE, (void *)p_log_buffer, LOG_BUFFER_SIZE);
	}

	p_log_buffer->size = LOG_BUFFER_SIZE - ((uint8_t *)p_log_buffer->addr - (uint8_t *)p_log_buffer);
	...
	p_log_buffer->magic = 0x676f6c7544525053;

	memset((void *)p_log_buffer->addr, 0, p_log_buffer->size);

#ifdef CONFIG_DTS_MEM_LAYOUT
	log_buffer_enabled_flag = 1;
#endif

	//debug("log base %lx, log buffer addr %x, log buffer size %x\n", p_log_buffer, p_log_buffer->addr, p_log_buffer->size);
	return (0);
}

#ifdef CONFIG_DTS_MEM_LAYOUT
static int get_log_buffer()
{
	unsigned long buf_base = 0, buf_size = 0;
	get_buffer_base_size_from_dt("heap@8", &buf_base, &buf_size);
	p_log_buffer = (LOG_BUFFER *)buf_base;
	log_buffer_enabled_flag = 1;
	debug("log base %p\n", p_log_buffer);
...
#endif
#if defined(CONFIG_GICV2) && defined(CONFIG_USE_IRQ)
...
u32 tuboot_s = 0x5a5a1234;
u32 tuboot_e = 0x5a5a5678;
#endif
...
void board_init_f(ulong bootflag)
{
	...
	gd->fb_base = addr; */  //fb_base is probed from dts in sprdfb_probe function
	...
	gd->no_relloc_malloc_addr = addr_sp;
#endif
	/*
	...
	if (new_fdt) {
		...
		gd->fdt_blob = new_fdt;
		debugf("dtb relocation %p size %lu\n", gd->fdt_blob, fdt_size);
	}
...
#if defined(CONFIG_SPRD_LOG) && defined(CONFIG_DTS_MEM_LAYOUT)
	get_log_buffer();
#endif
...
#else
	malloc_start = gd->no_relloc_malloc_addr;
#endif
