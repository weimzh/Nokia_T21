...
struct rtc_ops {
	...
	/**
	 * poweroff_alarm() - get the poweroff_alarm
	 *
	 * read poweroff_alarm from the RTC device.
	 *
	 * @dev:	Device to read from
	 * @check_int:   Flag to decide whether to judge alarm interrupt
	 * @poweroff:	place to put poweroff_alarm is read
	 * @return 0 if OK, -ve on error
	 */
	int (*poweroff_alarm)(struct udevice *dev, bool check_int, bool *poweroff);
...
/**
 * dm_rtc_get_poweroff_alarm() - Read the poweroff_alarm an RTC
 *
 * @dev:	Device to read from
 * @check_int:   Flag to decide whether to judge alarm interrupt
 * @poweroff:	place to put poweroff_alarm is read
 * @return 0 if OK, -ve on error
 */
int dm_rtc_get_poweroff_alarm(struct udevice *dev, bool check_int, bool *poweroff);
