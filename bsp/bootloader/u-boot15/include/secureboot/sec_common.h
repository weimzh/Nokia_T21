...
typedef struct{
	...
	uint64_t flag;		//0:wcn  1:gps  2:normal images
}imgToVerifyInfo;
...
typedef struct{
    uint32_t img_ver[AVB_MAX_NUMBER_OF_ROLLBACK_INDEX_LOCATIONS];
    uint32_t update_flag;
    uint32_t ab_slot_flag;
}VbootVerInfo;
...
#if defined (CONFIG_SOC_SHARKLE)||defined (CONFIG_SOC_SHARKL3)
...
#elif defined (CONFIG_SOC_QOGIRL6)||defined (CONFIG_SOC_QOGIRN6PRO)
#define IRAM_BEGIN 0x65000800
#else
...
typedef struct {
    ...
    uint8_t   reserved[444];    // 444 + 17*4 = 512
} sys_img_header;
