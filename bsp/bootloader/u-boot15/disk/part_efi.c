...
static void prepare_backup_gpt_header(gpt_header *gpt_h)
{
	...
	gpt_h->partition_entry_lba =
			cpu_to_le64(le64_to_cpu(gpt_h->my_lba) - 32);
...
#ifdef CONFIG_PARTITION_UUIDS
	uuid_bin_to_str(pgpt_pte[i].unique_partition_guid.b, info->uuid,
				UUID_STR_FORMAT_GUID);
#endif
		...
		gpt_header *gpt_h, gpt_entry *gpt_e)
{
	...
	debug("Primary GPT successfully written to block device!\n");
	return 0;
		...
		char *str_guid, int parts_count)
{
	...
	gpt_h->last_usable_lba = cpu_to_le64(dev_desc->lba - 4097);
...
int write_mbr_and_gpt_partitions(block_dev_desc_t *dev_desc, void *buf)
{
	...
	while (gpt_e->starting_lba != 0) {
		for (i = 0; i < strlen("userdata"); i++) {
			dpart_name[i] = (efi_char16_t)gpt_e->partition_name[i];
		}

		if (!strcmp(dpart_name, "userdata")) {
			gpt_e->ending_lba = gpt_h->last_usable_lba;
			break;
		}
		gpt_e += 1;
