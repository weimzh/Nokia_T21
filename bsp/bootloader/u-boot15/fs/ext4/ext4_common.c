...
int ext4fs_get_file_inode_no(const char *dirname, char *dname, int flags)
{
	int file_inode_no;
	int parent_inode_no;
	...
	parent_inode_no = ext4fs_get_parent_inode_num(dirname, dname, flags);

	printf("parent_inode_no is %d\n", parent_inode_no);

	if((parent_inode_no == -1) || (parent_inode_no == -2))
		goto fail;

	if(ext4fs_iget(parent_inode_no, parent_inode))
		goto fail;
	
	file_inode_no = search_dir(parent_inode, dname);
...
int get_inode_no(const char *dirname,char *dname,int *par_ino_no)
{
	int i;
	int depth = 0;
	int parent_inode_no;
	...
	for (i = 1; i < depth; i++) {
		...
		if (matched_inode_no == -1) {
			...
			if (ptr[i + 1] != NULL) {
					parent_inode_no = matched_inode_no;
					memset(parent_inode, '\0',
				       sizeof(struct ext2_inode));
				if (ext4fs_iget(matched_inode_no,
						parent_inode)) {
	...
	printf("dir inode is %d\nparent inode is%d\n", matched_inode_no, parent_inode_no);
	*par_ino_no = parent_inode_no;
	memcpy(dname, ptr[i], strlen(ptr[i]));
fail:
	free(depth_dirname);
	free(parse_dirname);
...
void allocate_from_direct(struct ext2_inode *file_inode,
                                        unsigned int ori_blk,
                                        unsigned int add_blk,
                                        unsigned int *total_add_blk)
{
	short i;
	long int direct_blockno;
	unsigned int no_blks_reqd = 0;

	for (i = ori_blk; add_blk && i < INDIRECT_BLOCKS; i++) {
		direct_blockno = ext4fs_get_new_blk_no();
		if (direct_blockno == -1) {
			printf("no block left to assign\n");
			return -1;
		...
		file_inode->b.blocks.dir_blocks[i] = direct_blockno;
		debug("DB %ld: %u\n", direct_blockno, add_blk);

		add_blk--;
	}

	alloc_single_indirect_block(file_inode, &add_blk, &no_blks_reqd);
	alloc_double_indirect_block(file_inode, &add_blk, &no_blks_reqd);
	alloc_triple_indirect_block(file_inode, &add_blk, &no_blks_reqd);
	*total_add_blk += no_blks_reqd;

}

void allocate_from_single_indirect(struct ext2_inode *file_inode,
                                        unsigned int ori_blk,
                                        unsigned int add_blk,
                                        unsigned int *total_add_blk)
{
	...
	si_blockno = file_inode->b.blocks.indir_block;

	si_block = zalloc(fs->blksz);

	ext4fs_devread((lbaint_t)si_blockno * fs->sect_perblk, 0, fs->blksz, si_block);
	memset(si_block,'\0',fs->blksz);
	si_buffer = si_block;
	offset_blk = ori_blk - 12;
	si_buffer += offset_blk;

	for (i = offset_blk; i < (fs->blksz / sizeof(int)); i++){
		actual_block_no = ext4fs_get_new_blk_no();
	...
	put_ext4(((uint64_t) ((uint64_t)si_blockno * (uint64_t)fs->blksz)), si_block , fs->blksz);
...
void allocate_from_double_indirect(struct ext2_inode *file_inode,
                                        unsigned int ori_blk,
                                        unsigned int add_blk,
                                        unsigned int *total_add_blk)
{
	...
	long int actual_block_no;
	long int di_parent_blockno;
	long int di_child_blockno;
	char *di_parent_block;
	char *di_child_block;
	unsigned int *di_parent_buffer;
	unsigned int *di_child_buffer;

	struct ext_filesystem *fs = get_fs();
	unsigned int no_blks_reqd = 0;


	di_parent_blockno = file_inode->b.blocks.double_indir_block;

	di_parent_block = zalloc(fs->blksz);

	ext4fs_devread((lbaint_t)di_parent_blockno * fs->sect_perblk, 0, fs->blksz, di_parent_block);

	di_parent_buffer = di_parent_block;

	offset_blk = ori_blk - 1036;
        parent_offset = offset_blk / 1024 ;
        child_offset = offset_blk % 1024;

	if(child_offset == 0){
		di_parent_buffer += parent_offset;

		for (i = parent_offset; i < (fs->blksz / sizeof(int)); i++) {
			di_child_blockno = ext4fs_get_new_blk_no();
			if (di_child_block == -1) {
				printf("no block left to assign\n");
				goto fail;
			}
			di_child_block = zalloc(fs->blksz);
			if (!di_child_block)
				goto fail;

			di_child_buffer = di_child_block;
			*di_parent_buffer = di_child_blockno;
			di_parent_buffer++;
			no_blks_reqd++;

			status = ext4fs_devread((lbaint_t)di_child_blockno *
						fs->sect_perblk, 0,
						fs->blksz,
						(char *)di_child_block);
			...
			memset(di_child_block, '\0', fs->blksz);
			/* filling of actual datablocks for each child */
			for (j = 0; j < (fs->blksz / sizeof(int)); j++) {
				actual_block_no = ext4fs_get_new_blk_no();
				if (actual_block_no == -1) {
				...
				*di_child_buffer = actual_block_no;
			...
			/* write the block  table */
			put_ext4(((uint64_t) ((uint64_t)di_child_blockno * (uint64_t)fs->blksz)),
				 di_child_block, fs->blksz);
			free(di_child_block);
			di_child_block = NULL;
		...
		put_ext4(((uint64_t) ((uint64_t)di_parent_blockno * (uint64_t)fs->blksz)),
			 di_parent_block, fs->blksz);


	}else{
		di_parent_buffer += parent_offset;
		di_child_blockno = *di_parent_buffer;
		...
		goto loop;
                ...
                for (i; i < (fs->blksz / sizeof(int)); i++) {
                        ...
                        *di_parent_buffer = di_child_blockno;
                        ...
                        memset(di_child_buffer, '\0', fs->blksz);
                        /* filling of actual datablocks for each child */
                        for (j = 0; j < (fs->blksz / sizeof(int)); j++) {
loop:				actual_block_no = ext4fs_get_new_blk_no();
                                if (actual_block_no == -1) {
                                        printf("no block left to assign\n");
                                        goto fail;
                                }
...
fail:
	free(di_child_block);
	free(di_parent_block);

}
void allocate_from_triple_indirect(struct ext2_inode *file_inode,
                                        unsigned int ori_blk,
                                        unsigned int add_blk,
                                        unsigned int *total_add_blk)
{
...
void ext4fs_allocate_append_blocks(struct ext2_inode *file_inode,
					unsigned int ori_blk,
					unsigned int add_blk,
					unsigned int *total_add_blk)
{
	...
	allocate_from_direct(file_inode, ori_blk, add_blk,total_add_blk);

	}else if (ori_blk < 1037){

	allocate_from_single_indirect(file_inode, ori_blk, add_blk,total_add_blk);

	}else if (ori_blk < 1049613){

	allocate_from_double_indirect(file_inode, ori_blk, add_blk,total_add_blk);

	}else{

	allocate_from_triple_indirect(file_inode, ori_blk, add_blk,total_add_blk);
