...
#define DONT_RESIZE 0
#define RESIZE_IN_MEM 1
#define RESIZE_FROM_EMMC 2

typedef int (*f2fs_devcie_callback)(void* handle, uint64_t buf_size, uint64_t offset, void *buf);
...
//#endif
	...
	void *device_handle;
	f2fs_devcie_callback write;
	f2fs_devcie_callback read;
	uint64_t cache_buffer_size;
	uint64_t cache_buffer_dirty_end;
	unsigned char *cache_buffer;
};

struct resize_configuration {
	uint16_t resize_flag;
	uint64_t buffer_begin;
	uint64_t buffer_end;
	uint64_t valid_meta_size;
};
...
extern int f2fs_init_configuration(uint64_t dev_total_sectors, uint32_t dev_sector_size);
extern void f2fs_init_resize_configuration();
...
extern struct resize_configuration g_resize_config;
