...
struct resize_configuration g_resize_config;
 ...
 * device information
 */
int f2fs_init_configuration(uint64_t device_total_size, uint32_t device_sector_size)
{
	memset(&c, 0, sizeof(c));
	c.dbg_lv = 5;
	c.bytes_reserved = 16384; //must same as /system/core/fs_mgr.cpp
	c.total_sectors = device_total_size / device_sector_size;
	c.sector_size = device_sector_size;
	c.sectors_per_blk = DEFAULT_SECTORS_PER_BLOCK;
	c.blks_per_seg = DEFAULT_BLOCKS_PER_SEGMENT;
	c.wanted_total_sectors = -1;
	c.wanted_sector_size = 512;
...
void f2fs_init_resize_configuration()
{
	g_resize_config.resize_flag = DONT_RESIZE;
	g_resize_config.buffer_begin = 0;
	g_resize_config.buffer_end = 0;
	g_resize_config.valid_meta_size = 0;
}
...
int get_device_info(int i)
{
	...
	if (c.bytes_reserved) {
		...
		if (c.bytes_reserved % dev->sector_size) {reserved_sectors++;}
