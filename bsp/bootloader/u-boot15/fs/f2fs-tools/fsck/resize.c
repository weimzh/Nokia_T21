...
static uint64_t calculate_valid_data(struct f2fs_sb_info *sbi)
{
	uint64_t total_valid_blks = 0;
	struct seg_entry *se;
	int i, j;
	for (i = TOTAL_SEGS(sbi) - 1; i >= 0; i--) {
		se = get_seg_entry(sbi, i);
	...
	debugf("total_valid_blks is %llu\n", total_valid_blks);
	return total_valid_blks;
}
...
static int f2fs_resize_grow(struct f2fs_sb_info *sbi)
{
	...
	int err = -1;
	uint64_t total_meta_segments = 0;
	uint64_t new_total_meta_size = 0;
	...
	/* check whether the size of buffer is large enough */
	total_meta_segments = get_newsb(segment_count_ckpt) +
                get_newsb(segment_count_sit) +
                get_newsb(segment_count_nat) +
                get_newsb(segment_count_ssa);

	new_total_meta_size = total_meta_segments << (get_sb(log_blocksize) + get_sb(log_blocks_per_seg));
	g_resize_config.valid_meta_size = new_total_meta_size;
	debugf("new_total_meta_size is 0x%llx, buffer_size is 0x%llx\n", new_total_meta_size, c.cache_buffer_size);
	old_data_size = (calculate_valid_data(sbi)) << get_sb(log_blocksize);
	debugf("old_data_size is 0x%llx\n", old_data_size);
	if (new_total_meta_size + old_data_size > c.cache_buffer_size) {
		errorf("small buffer! please config dts!\n");
		return -1;
	}
	...
	err = -1;
	MSG(0, "defragement: old=0x%x, new=0x%x end=0x%x\n", old_main_blkaddr, new_main_blkaddr, end_blkaddr);
