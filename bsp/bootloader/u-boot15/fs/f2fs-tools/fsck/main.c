...
int f2fs_resize_main(uint64_t device_total_size, uint32_t device_sector_size, f2fs_devcie_callback write, f2fs_devcie_callback read, void *device_handle, unsigned char *cache_buffer, uint64_t cache_buffer_size, uint64_t write_size)
{
	...
	if (g_resize_config.resize_flag == RESIZE_IN_MEM) {
		debugf("reserved buffer is %llx, write_size is %llx\n", g_resize_config.buffer_begin, write_size);
		memset(c.cache_buffer, 0, write_size);
		memcpy(c.cache_buffer, g_resize_config.buffer_begin, write_size);
		c.cache_buffer_size -= write_size;
	} else {
	...
	debugf("vaild data size is 0x%llx\n", g_resize_config.valid_meta_size);
	c.write(c.device_handle, c.cache_buffer_size, (uint64_t)0LL, c.cache_buffer);
	printf("\nDone.\n");
	return 0;

out_err:
	if (g_resize_config.resize_flag == RESIZE_IN_MEM) {
                printf("resize failed, try to download reserved img, write_size is 0x%llx, from 0x%llx\n", write_size, g_resize_config.buffer_begin);
                c.write(c.device_handle, write_size, (uint64_t)0LL, g_resize_config.buffer_begin);
        }
	if (sbi->ckpt)
		free(sbi->ckpt);
	if (sbi->raw_super)
