...
export ARCH CPU BOARD VENDOR SOC CPUDIR BOARDDIR DIRARCH
...
KCFLAGS += -DAVB_ENABLE_DEBUG -DAVB_COMPILATION
endif
...
KBUILD_CFLAGS += -DDEVINFO_PROJECT='"$(DEVINFO_PROJECT)"'
KBUILD_CFLAGS += -DDEVINFO_PRODUCT='"$(DEVINFO_PRODUCT)"'
KBUILD_CFLAGS += -DDEVINFO_MODEL='"$(DEVINFO_MODEL)"'
KBUILD_CFLAGS += -DDEVINFO_VERSION='"$(DEVINFO_VERSION)"'

KBUILD_CFLAGS += -DANTIROLLBACK_HW='"$(ANTIROLLBACK_HW)"'
KBUILD_CFLAGS += -DANTIROLLBACK_VBMETA='"$(ANTIROLLBACK_VBMETA)"'
KBUILD_CFLAGS += -DANTIROLLBACK_VBMETA_SYSTEM='"$(ANTIROLLBACK_VBMETA_SYSTEM)"'
...
UBOOTINCLUDE    := \
		...
		-I$(srctree)/arch/$(ARCH)/lib/mv \
		-I$(srctree)/lib/secureboot/avb/libavb \
...
libs-$(CONFIG_CMD_NAND) += drivers/mtd/nand_sprd/
...
libs-$(CONFIG_DWC_OTG) += drivers/usb/dwc_otg/
libs-$(CONFIG_USB_DWC3) += drivers/usb/dwc3/
...
libs-$(CONFIG_SECBOOT) += lib/secureboot/
ifdef CONFIG_X86
libs-y += lib/sec_vm/
else
libs-y += lib/trustzone/
endif
libs-$(CONFIG_API) += api/
libs-$(CONFIG_HAS_POST) += post/
libs-y += common/loader/
libs-y += common/dloader/
#libs-y += test/
#libs-y += test/dm/
libs-$(CONFIG_UT_ENV) += test/env/
libs-$(CONFIG_CMD_MEMTEST) += test/memtest/
...
ifeq ($(CONFIG_SANSA_SECBOOT),y)
# Add DX lib
PLATFORM_LIBDX = -L$(srctree)/lib/secureboot/sansa/lib -ldx_sbrom
else
PLATFORM_LIBDX =
endif
PLATFORM_LIBS += $(PLATFORM_LIBDX)
