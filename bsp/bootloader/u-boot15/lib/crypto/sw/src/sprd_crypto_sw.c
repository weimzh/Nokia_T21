		...
		sprd_rsa_padding_t padding, int32_t *result)
{
	...
	uint8_t temp1[256] __attribute__ ((aligned (8))) = "0", temp2[256] __attribute__ ((aligned (8))) = "0";
	uint32_t temp1_len = 0, temp2_len = 0;
	int32_t is_valid = 0;
	...
	if (temp1_len == 0 || temp1_len>>24 & 0xFF) {
		errorf("rsa pubdec failed \n");
		return temp1_len;
	}
	...
	switch (padding.type) {
		...
		case SPRD_RSASSA_PKCS1_V1_5:
			temp2_len = 256;
			err = sprd_pkcs1_v1_5_decode_sw(temp1, temp1_len,
		...
		case SPRD_RSASSA_PKCS1_PSS_MGF1:
			err = sprd_pkcs1_pss_decode_sw(dig, dig_size,
					temp1, temp1_len,
...
sprd_crypto_err_t sprd_sha256_test(void)
{
	...
	for (i = 0; i < (int)(sizeof(tests) / sizeof(tests[0])); i++) {
		...
		sec_memset(out, 0, 32);
		sha256_csum_wd_ce(in, tests[i].msg_len, out, 0);
...
int sprd_special_test(void) {
	...
	uint8_t rsa_temp[32] = {0};

	sha256_csum_wd_sw((const unsigned char*)test_hash.msg, test_hash.msg_len, hash_temp, 0);
	if (sec_memcmp(test_hash.hash, hash_temp, 32)) {
		errorf("sha256_csum_wd ce failed!\n");
		return -1;
	}

	err = RSA_Verify(test_rsa_pkcs1.key_e,
