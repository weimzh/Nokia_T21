...
{
	u8 counter = 0xFF & SYSTEM_CURRENT_CLOCK ;
	u8 counter1 = 0xFF00 & SYSTEM_CURRENT_CLOCK;
	size_t pos, plen;
...
int prand_gen(u8 *rand, size_t rand_len)
{
	...
	if (0 == sha1_prf(key, sizeof(key), "sprd_sha1_rng", "data", 4, rand, rand_len)) {
		printf("sha1-rng success!\n");
		return 0;
	} else {
		printf("sha1-rng failed!\n");
		return (-1);
	};
