 ...
 * Parameter:
 * [IN]pubExp: public expoenent, should be RSA_3 or RSA_F4, to be compatible wi * th rsa->e from openssl, the pubExp should be BIG-ENDIAN like [0~3]={0x00,0x01,0x00,0x01}
 * [IN]bitLen_N: 2048, ...
 ...
 * [IN]from: clear text to be signed, MAX length is bitLen_N>>3-11 due to will be padded using pkcs1
 ...
 *   false: sign failed
 */
int RSA_Sign(unsigned char *prvExp, unsigned char *mod_N, int bitLen_N, unsigned char *from, int flen, unsigned char *to);
int RSA_Sign_pss(unsigned char *prv_E, unsigned char *mod_N, int bitLen_N, unsigned char *from,
                    int flen, unsigned char *to, sprd_crypto_algo_t hash_type, int sLen);
 ...
 *   false: sign failed
 */
int RSA_PrvEnc_No_Pad(unsigned char *prvExp, unsigned char *mod_N, int bitLen_N, unsigned char *from, int flen, unsigned char *to);

/*
 * Decrypt using public key with pkcs1 padding,  Means verification
 * Parameter:
 * [IN]pubExp: public expoenent, should be RSA_3 or RSA_F4, to be compatible with rsa->e from openssl, the pubExp should be BIG-ENDIAN like [0~3]={0x00,0x01,0x00,0x01}
 * [IN]mod_N: modulus ,length should be bitLen_N>>3 bytes
 * [IN]bitLen_N: bit length of mod_N
 ...
 */
int RSA_Verify(unsigned char *pubExp, unsigned char *mod_N, int bitLen_N, unsigned char *from, unsigned char *to);
 ...
 */
int RSA_PubDec_No_Pad(unsigned char *pubExp, unsigned char *mod_N, int bitLen_N, unsigned char *from, unsigned char *to);
 ...
 *   false: encrypt failed
 */
int RSA_PubEnc_No_Pad(unsigned char *pubExp, unsigned char *mod_N, int bitLen_N, unsigned char *from, int flen, unsigned char *to);
 ...
 *   Bytes length of clear data in "to" buffer, MAX is  bitLen_N>>3-11
 */
int RSA_PrvDec_No_Pad(unsigned char *prvExp, unsigned char *mod_N, int bitLen_N, unsigned char *from, unsigned char *to);
