...
unsigned int sprd_get_version(sw_version_id_t counterId, uint64_t* swVersion)
{
	unsigned int ret = SEC_EFUSE_OK;
	unsigned int ver_part1 = 0;
	unsigned int ver_part2 = 0;
	uint64_t ver64 = 0;
	...
	switch(counterId) {
		case SW_VERSION_COUNTER1:
			ret = sec_efuse_read(SEC_VERSION_BLOCK, &ver_part1);
			ret = sec_efuse_read(SEC_VERSION1_BLOCK, &ver_part2);
			ver64 = (uint64_t)ver_part2 << 32;
			*swVersion = ((uint64_t)ver_part1 |ver64);
...
unsigned int sprd_set_version(sw_version_id_t counterId, uint64_t swVersion)
{
	unsigned int ret = SEC_EFUSE_OK;
	unsigned int ver_part1 = 0;
	unsigned int ver_part2 = 0;
	...
	switch(counterId) {
		case SW_VERSION_COUNTER1:
			ver_part1 = (unsigned int)(swVersion & 0xffffffff);
			ver_part2 = (unsigned int)((swVersion & 0xffffffff00000000) >> 32);
			ret = sec_efuse_write_and_check(SEC_VERSION_BLOCK, ver_part1);
			ret = sec_efuse_write_and_check(SEC_VERSION1_BLOCK, ver_part2);
...
unsigned int sprd_get_lcs(unsigned *pLcs)
{
    ...
    if ((huk_bit1 != 0) || (rotpk0_bit1 !=0) || (rotpk1_bit1 != 0)){
        ret = sec_efuse_read(LOCK_BIT_BLOCK + 1, &read_data1);
        if (ret)
            return ret;
    }
 ...
 * enable rotpk1
 */
unsigned int sprd_set_rotpk1_enable(void)
{
    /* set Device life cycle state register's 1 bit */
    return sec_efuse_set_bit(CYCLE_STATE_BLOCK, 1);
}
