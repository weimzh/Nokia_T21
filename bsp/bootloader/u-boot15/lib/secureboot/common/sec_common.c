...
//20220614, Added by zhu_jun for red eio warning screen when avb verify failed begin
static void red_eio_warning_enter_fastboot(void)
{
	display_state_verified_eio_red();
	debugf("enter fastboot mode.\n");
...
fail:
	...
	//20220614, Added by zhu_jun for red eio warning screen when avb verify failed begin
	/* The code runs here and the phone becomes bricked,
	   so modify the code to fix this problem.
	*/
	red_eio_warning_enter_fastboot();
	//20220614, Added by zhu_jun for red eio warning screen when avb verify failed end
	while (1);
}
