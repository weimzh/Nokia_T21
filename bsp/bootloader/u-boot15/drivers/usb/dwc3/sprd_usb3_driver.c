...
static usb3_dma_desc_t	g_epn_out_desc[2] __attribute__((aligned (128)));
...
static unsigned char *usb3_out_endpoint = NULL;
static uint32_t g_recv_pkt_size = MAX_RECV_LENGTH;

PUBLIC __attribute__((aligned (128)))  unsigned char usb_out_endpoint_buf[RECV_PKT_SIZE + MAX_RECV_LENGTH];
                                 ...
                                 usb3_pcd_ep_t *ep, uint32_t event)
{
	if (ep == pcd->out_ep) {
		...
		g_out_len = g_recv_pkt_size -
		            (desc->status >> DSCSTS_XFRCNT_SHIFT
		             & DSCSTS_XFRCNT_BITS >> DSCSTS_XFRCNT_SHIFT);

		/* Sync transmit buffer */
		usb3_flush_cache((phys_addr_t)usb3_out_endpoint, g_recv_pkt_size, 1);
 ...
 */
void usb_receive(unsigned char* buf, unsigned int len)
{
	...
	usb3_fill_desc(ep->ep.dma_desc, (phys_addr_t)buf,
	               len, DSCCTL_TRBCTL_NORMAL,
	               DSCCTL_LST_BIT, 1);

	usb3_out_endpoint = buf;
	g_recv_pkt_size = len;
...
void usb3_start_receive_pkt(unsigned char* buf, unsigned int len)
{
	usb3_pcd_t *pcd = &g_usb3_dev.pcd;
	usb3_pcd_ep_t *ep = pcd->out_ep;
	uint8_t tri;
	...
	/* Fill in Bulk In TRB */
	usb3_fill_desc(ep->ep.dma_desc, buf,
		len, DSCCTL_TRBCTL_NORMAL,
		zlp ? 0 : DSCCTL_LST_BIT, 1);

	if (zlp)
		usb3_fill_desc(ep->ep.dma_desc + 1, usb_out_endpoint_cmd,
			(unsigned int)MAX_RECV_LENGTH, DSCCTL_TRBCTL_NORMAL,
			DSCCTL_LST_BIT, 1);
...
void usb_send(unsigned char *buf, unsigned int len)
{
	...
	if (!usb_support_raw_data_proc()) {
		usb_receive(usb_out_endpoint_cmd, (unsigned int)MAX_RECV_LENGTH);
	}
	wait_usb3_send_done();
}
...
uint8_t usb_get_entry()
{
	...
	if(g_out_len == 1) {
		g_out_len = 0;
		return usb_out_endpoint_cmd[0];
	}
	return 0;
}

int usb3_receive_pkt_done(unsigned int len)
{
	uint32_t rx_len;
...
loop:
		...
		if (g_out_len) {
			if (hdlc_disabled) {
				int count = g_out_len;
				memcpy(dest,usb3_out_endpoint+1,count-2);
				g_out_len = 0;;
				return count-2;
			}
			for (src = usb3_out_endpoint; g_out_len--; src++) {
				switch(pkt_flag) {
				case PKT_NONE:
					if (*src == HDLC_FLAG)
						pkt_flag = PKT_HEAD;
...
{
	usb3_start_receive_pkt(buf, HALF_PKT_SIZE);
	return 0;
}

int usb_rx_data_from_linklist(unsigned int len)
{
	return usb3_receive_pkt_done(len);
}
