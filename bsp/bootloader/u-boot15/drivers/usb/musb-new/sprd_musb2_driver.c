...
extern uint8_t usb_get_dload_sys_linux(void);
...
static uint32_t g_dma_node_length = MAX_RECV_LENGTH;
...
void prepare_linklist_node (uint32_t dir,int ep, unsigned int transfer_size, unsigned int *buffer, unsigned char list_end, unsigned int index)
{
	...
	g_dma_node_length = MAX_RECV_LENGTH;
...
void prepare_linklist_multi_nodes(uint32_t dir,int ep, unsigned int transfer_size, unsigned char *buf)
{
	...
	if (usb_get_dload_sys_linux()) {
		/* prepare actual length for last packet when dload in linux */
		prepare_linklist_node (dir, ep, buf_length, (unsigned int *)(buf + offset), 0x01, link_list_index);
		g_dma_node_length = buf_length;
	} else {
		/* prepare MAX_RECV_LENGTH for last packet when dload in windows */
		if (buf_length == packet_len) {
			prepare_linklist_node (dir, ep, packet_len, (unsigned int *)(buf + offset), 0x0, link_list_index);
			link_list_index++;
			offset += packet_len;
			prepare_linklist_node (dir, ep, 0, (unsigned int *)(buf + offset), 0x01, link_list_index);
		} else
...
LOCAL void usb_EP_out_handle (void)
{
	...
	recv_length = g_dma_node_length - reg32_blk_len;
