...
#if defined(CONFIG_ADIE_SC2720) || defined(CONFIG_ADIE_SC2730) || defined(CONFIG_ADIE_UMP9620)
...
struct sprd_adc_cal small_scale_cal = {
	1000, 3413,
	100, 341,
	SPRD_AUXADC_CAL_NO,
};

int32_t adc_rawresults[ADC_MESURE_NUMBER + 1] = { 0 };
int32_t adc_rawdata = 0;
int32_t adc_batraw = 0, adc_batvol = 0, adc_smallraw = 0, adc_smallvol = 0;
...
static struct pmic_adc_para para = {
	.small_cal_efuse_blk_1v0 = 33,
	.small_cal_efuse_blk_0v1 = 32,
	.big_cal_efuse_blk_3v6 = 29,
	.big_cal_efuse_blk_4v2 = 28,
	.blk_width = 16,
	.cal_efuse_bit = 16,
};
...
static void sc27xx_adc_data_qusort(u32 a[], int N)
{
	int i, j, temp = 0;
	for (i = 0; i < N; i++) {
		for (j = i+1; j < N; j++) {
...
static int pmic_adc_set_isen(u8 enable, int isen)
{
	...
	if (enable) {
		...
		sci_adi_set(ANA_REG_GLB_IB_CTRL, BIT_BATDET_CUR_I(isen_bit));
#else
...
#if defined(CONFIG_ADIE_UMP9620)
		sci_adi_clr(ANA_REG_GLB_IB_CTRL, BIT_BATDET_CUR_I(~0));
#else
				   ...
				   int isen)
{
	...
	if (0 == pmic_adc_get_values(channel, scale, num, adc_rawresults)) {
		sc27xx_adc_data_qusort(adc_rawresults, num);
		values =
		    pmic_adc_data_average(&adc_rawresults[num / 5],
					  (num - num * 2 / 5));
	}
	udelay(500);
	pmic_adc_set_current(0, 0);

	for (i = 0; i < (ARRAY_SIZE(adc_rawresults) - 1) ; i++) {
		debugf("%d\n", adc_rawresults[i]);
	}
...
static int pmic_adc_get_ratio(uint16_t channel, int scale, int mux)
{
	...
	case ADC_CHANNEL14:
		...
		case 0:
			return RATIO(68, 900);
		case 1:
			return RATIO(136, 3519);
		case 2:
			return RATIO(68, 2340);
		case 3:
			return RATIO(68, 3654);
		default:
			return RATIO(1, 1);
	...
	default:
		...
		case 1:
			return RATIO(1000, 1955);
		case 2:
			return RATIO(1000, 2600);
		case 3:
			return RATIO(1000, 4060);
		default:
			return RATIO(1, 1);
		}
	}
...
{
	uint32_t deta_1v0 = 0, deta_0v1 = 0;

	if (NULL == p_cal_data) {
		sprd_err("ERR!adc small cal p_cal_data is NULL\n");
		return -1;
	}
	/* adc 1.0V */
	deta_1v0 = sprd_pmic_efuse_read_bits(BITSINDEX(para.small_cal_efuse_blk_1v0, 0),
				 para.cal_efuse_bit);
	if (!deta_1v0) {
		sprd_err("ERR!adc small cali efuse deta_1v0:0x%x\n", deta_1v0);
		return -1;
	}

	/* adc 0.1V */
	deta_0v1 = sprd_pmic_efuse_read_bits(BITSINDEX(para.small_cal_efuse_blk_0v1, 0),
				 para.cal_efuse_bit);
	if (!deta_0v1) {
		sprd_err("ERR!adc small cali efuse deta_0v1:0x%x\n", deta_0v1);
		return -1;
	}

	/* adc 1.0V */
	p_cal_data[0] = (deta_1v0 & 0xfff0) >> 4;
	debugf("adc deta_1v0:0x%x,p_cal_data[0]:0x%x\n", deta_1v0, p_cal_data[0]);

	/* adc 0.1V */
	p_cal_data[1] = (deta_0v1 & 0xfff0) >> 4;
	debugf("adc deta_0v1:0x%x,p_cal_data[1]:0x%x\n", deta_0v1, p_cal_data[1]);
...
#if defined(CONFIG_ADIE_SC2723) || defined(CONFIG_ADIE_SC2723S)
	/* adc 0.1V */
	adc_data = ((deta & 0x00FF) + para.small_adc_p1 - para.adc_data_off) * 4;
...
{
	uint32_t deta_3v6 = 0, deta_4v2 = 0;

	if (NULL == p_cal_data) {
		sprd_err("ERR!adc bat cal p_cal_data is NULL\n");
		return -1;
	}
	/* adc 4.2V */
	deta_4v2 = sprd_pmic_efuse_read_bits(BITSINDEX(para.big_cal_efuse_blk_4v2, 0),
                                             para.cal_efuse_bit);
	if (!deta_4v2) {
		sprd_err("ERR! adc bat cal deta_4v2:0x%x\n", deta_4v2);
		return -1;
	}
	/* adc 3.6V */
	deta_3v6 = sprd_pmic_efuse_read_bits(BITSINDEX(para.big_cal_efuse_blk_3v6, 0),
                                             para.cal_efuse_bit);
	if (!deta_3v6) {
		sprd_err("ERR! adc bat cal deta_3v6:0x%x\n", deta_3v6);
		return -1;
	}

	/* adc 4.2V */
	p_cal_data[0] = (deta_4v2 & 0xfff0) >> 4;
	debugf("adc bat deta_4v2: 0x%x, p_cal_data[0]: 0x%x\n", deta_4v2, p_cal_data[0]);
	/* adc 3.6V */
	p_cal_data[1] = (deta_3v6 & 0xfff0) >> 4;
	debugf("adc bat deta_3v6: 0x%x, p_cal_data[1]: 0x%x\n", deta_3v6, p_cal_data[1]);
...
int sprd_get_adc_bat_cal(uint32_t *p_cal_data)
{
	...
	deta = sprd_pmic_efuse_read_bits(BITSINDEX(para.big_cal_efuse_blk, 0),
				para.big_cal_efuse_bit);
	deta &= 0xFFFFFF;
	if ((!deta) || (p_cal_data == NULL)) {
		return -1;
	}
...
static int sprd_bat_scale_adc_efuse_get(void)
{
	...
	if (!sprd_get_adc_bat_cal(efuse_cal_data)) {
		big_scale_cal.p0_adc = efuse_cal_data[0];
		big_scale_cal.p1_adc = efuse_cal_data[1];
		big_scale_cal.cal_type = SPRDBIG_AUXADC_CAL_CHIP;
		debugf("bat scal adc,p0_vol:%d, p0_adc:%d, p1_vol:%d, p1_adc:%d, cal_type:%d\n",
			big_scale_cal.p0_vol, big_scale_cal.p0_adc, big_scale_cal.p1_vol,
			big_scale_cal.p1_adc, big_scale_cal.cal_type);
		return 0;
	}
	return -1;
}
...
static int sprd_small_scale_adc_efuse_get(void)
{
	...
	if (!sprd_get_adc_small_scale_cal(efuse_cal_data)) {
		small_scale_cal.p0_adc = efuse_cal_data[0];
		small_scale_cal.p1_adc = efuse_cal_data[1];
		small_scale_cal.cal_type = SPRDLIT_AUXADC_CAL_CHIP;
		debugf("small scal adc p0_vol:%d, p0_adc:%d, p1_vol:%d, p1_adc:%d, cal_type:%d\n",
			small_scale_cal.p0_vol, small_scale_cal.p0_adc, small_scale_cal.p1_vol,
			small_scale_cal.p1_adc, small_scale_cal.cal_type);
		return 0;
	}
	return -1;
}
...
static int sprd_small_scale_adc_efuse_get(void)
{
	...
	if (!sprd_get_adc_small_scale_cal(efuse_cal_data)) {
		small_scale_cal.p0_vol = efuse_cal_data[0] & 0xffff;
		small_scale_cal.p0_adc = (efuse_cal_data[0] >> 16) & 0xffff;
		small_scale_cal.p1_vol = efuse_cal_data[1] & 0xffff;
		small_scale_cal.p1_adc = (efuse_cal_data[1] >> 16) & 0xffff;
		small_scale_cal.cal_type = SPRDLIT_AUXADC_CAL_CHIP;
		debugf("adc efuse small cal %d,%d,%d,%d,cal_type:%d\n",
			small_scale_cal.p0_vol, small_scale_cal.p0_adc,
			small_scale_cal.p1_vol, small_scale_cal.p1_adc,
			small_scale_cal.cal_type);
		return 0;
	}
	return -1;
}
...
static int32_t sprd_small_scale_to_vol(int32_t adcvalue)
{
	...
	vol = small_scale_cal.p0_vol - small_scale_cal.p1_vol;
	vol = vol * (adcvalue - small_scale_cal.p1_adc);
	vol = vol / (small_scale_cal.p0_adc - small_scale_cal.p1_adc);
	vol = vol + small_scale_cal.p1_vol;
...
static int32_t sprd_small_scale_to_vol(int32_t adcvalue)
{
	...
	vol = small_scale_cal.p0_vol - small_scale_cal.p1_vol;
	vol = vol * (adcvalue - small_scale_cal.p0_adc);
	vol = vol / (small_scale_cal.p0_adc - small_scale_cal.p1_adc);
	vol = vol + small_scale_cal.p0_vol;
...
static int32_t sprd_bat_scale_adc_to_vol(int32_t adcvalue)
{
	...
	vol = vol * (adcvalue - big_scale_cal.p1_adc);
	vol = vol / (big_scale_cal.p0_adc - big_scale_cal.p1_adc);
	vol = vol + big_scale_cal.p1_vol;
				    ...
				    int32_t adcvalue)
{
	...
	m = vol * bat_numerators * (denominators);
	result = (m + n / 2) / n;
	debugf("small adc to vol, numerators: %d, denominators: %d, adc: %d, vol: %d\n",
		numerators, denominators, adcvalue, result);

	adc_smallraw = adcvalue;
	adc_smallvol = result;
