...
#include <libfdt.h>//<linux/libfdt.h>
...
{
	assert(ofnode_valid(node));
	if (ofnode_read_u32(node, propname, &def))
        	errorf("Node %s has bad/missing %s property\n", ofnode_get_name(node), propname);
	return def;
}
...
{
	assert(ofnode_valid(node));
	if (ofnode_read_u32(node, propname, (u32 *)&def))
		errorf("Node %s has bad/missing %s property\n", ofnode_get_name(node), propname);
