...
static inline unsigned int sprd_pll_read(const struct sprd_pll *pll,
					 u8 index)
{
	const struct sprd_clk_common *common = &pll->common;
	unsigned int val = 0;

	WARN_ON(index >= pll->regs_num)
	if (index >= pll->regs_num)
		return 0;

	val = readl((void *)(common->reg_base + common->reg + index * 4));

	return val;
}

static inline void sprd_pll_write(const struct sprd_pll *pll,
				  u8 index, u32 msk, u32 val)
{
	const struct sprd_clk_common *common = &pll->common;
	unsigned int reg;
	int ret = 0;
	...
	reg = readl((void *)(common->reg_base + common->reg + index * 4));
	if (!ret)
		writel((reg & ~msk) | val,
		       (void *)(common->reg_base + common->reg + index * 4));
}
...
const struct sprd_clk_ops sprd_pll_ops = {
