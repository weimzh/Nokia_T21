...
#include <sprd_pka.h>
#if defined(CONFIG_SOC_QOGIRL6)
#include <secureboot/sec_efuse_sharkl6_drv.h>
#endif
#if defined(CONFIG_SOC_QOGIRN6PRO)
#include <secureboot/sec_efuse_sharkl6pro_drv.h>
#endif
...
sprd_crypto_process(sprd_crypto_context_t *context) {
	...
	case SPRD_CRYPTO_AES:
		SPRD_CRYPTO_LOG_TRACE("SPRD_CE_AES\n");
		SPRD_CRYPTO_LOG_TRACE("key1 = %p, key1_len = %d\n", context->info.aes.key1, context->info.aes.key1_len);
		SPRD_CRYPTO_LOG_TRACE("key2 = %p, key2_len = %d\n", context->info.aes.key2, context->info.aes.key2_len);
		SPRD_CRYPTO_LOG_TRACE("iv = %p, iv_len = %d\n", context->info.aes.iv, context->info.aes.iv_len);
...
sprd_crypto_open(sprd_crypto_context_t* context) {
	...
	if (context->algo == SPRD_CRYPTO_AES_CBC_HUK
		|| context->algo == SPRD_CRYPTO_AES_CMAC_HUK
		|| context->algo == SPRD_CRYPTO_AES_CBC_KCE) {
		efuse_enable();
		SPRD_CRYPTO_LOG_TRACE("open efuse!\n");
	}
...
sprd_crypto_close(sprd_crypto_context_t* context) {
	if (context->algo == SPRD_CRYPTO_AES_CBC_HUK
		...
		|| context->algo == SPRD_CRYPTO_AES_CBC_KCE) {
		efuse_disable();
		SPRD_CRYPTO_LOG_TRACE("close efuse!\n");
	}
