 ...
 */
/*******************AON switch clock to 26M*****************/
#define AON_CLK_FREQ_384M             0X4
#define AON_CLK_FREQ_48M_DIV       0X300
#define AON_CLK_FREQ_26M                0x1
	if (IS_SD(mmc))
		sprd_host_info = get_sdcontrol_info(SD);
	else
		sprd_host_info = get_sdcontrol_info(EMMC);
#ifndef CONFIG_FPGA
	CHIP_REG_SET(sprd_host_info->baseclk_reg,
		     AON_CLK_FREQ_48M_DIV | AON_CLK_FREQ_384M);
#endif
/******************************************************/
...
void board_mmc_test(void)
{
	...
	emmc_write(0, j, 1, testbuf_write);
	emmc_erase(0, j, 1);
	emmc_read(0, j, 1, testbuf_read);
	...
	flag = 0;
	emmc_write(0, 1, 10, pc_buffer);
	memset(pc_buffer, 0, 5120);
	emmc_read(0, 1, 10, pc_buffer);
