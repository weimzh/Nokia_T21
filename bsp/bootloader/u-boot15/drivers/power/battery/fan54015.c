...
#ifdef CONFIG_DM_FAN54015
struct sprd_fan54015_dm_data {
	int i2c_num;
};

static struct sprd_fan54015_dm_data dm_data = {
	.i2c_num = -1,
};

struct udevice *charger;

static int sprd_dm_fan54015_i2c_init(void)
{
	int ret;
	u8 value;
...
static int fan54015_read_reg(int reg, u8 *value)
{
	u8 val;
	int ret;
	...
	*value = (u8)ret;
	debugf("######fan54015readreg reg = %d value = %d/%x\n", reg, ret, ret);
	return 0;
}
#else
...
#ifdef CONFIG_DM_FAN54015
	ret = sprd_dm_fan54015_i2c_init();
#else
	ret = sprd_fan54015_i2c_init();
#endif
	if (ret) {
		pr_err("%s:fan54015 i2c init fail\n", __func__);
		return;
	}
	//reg 6
...
#ifdef CONFIG_DM_FAN54015
static int sprd_fan54015_probe(struct udevice *dev)
{
	u32 i2c_id;
	int ret;

	ret = dev_read_u32(dev, "sprd,fan54015-i2c-bus", &i2c_id);
	if (ret) {
...
static int sprd_dm_fan54015_init(void)
{
	struct udevice *devp;
	int ret;
...
#ifdef CONFIG_DM_FAN54015
		ret = sprd_dm_fan54015_init();
#else
		ret = sprd_fan54015_i2c_init();
#endif
	if (ret) {
		pr_err("fan54015 i2c init failed\n");
		return;
	}
...
#ifdef CONFIG_DM_FAN54015
static const struct udevice_id sprd_fan54015_ids[] = {
	{.compatible = "sprd,fan54015-charger"},
	{ }
};

U_BOOT_DRIVER(fan54015) = {
	.name = "sprd-fan54015",
	.id = UCLASS_CHARGER,
	.of_match = sprd_fan54015_ids,
	.probe = sprd_fan54015_probe,
};
