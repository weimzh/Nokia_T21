...
static void div_to_clk(u32 clk_div)
{
	switch (clk_div) {
	case CLK_DPI_DIV6:
		sci_glb_write(REG_ANLG_PHY_G1_ANALOG_MIPI_DSI_4LANE_DSI_ISO_SW, 0x0, 0xFFFF);
		sci_glb_write(REG_AP_CLK_CORE_CGM_DISPC0_DPI_CFG, BIT(0) | BIT(2), BIT(0) | BIT(1) | BIT(2));
		break;
	case CLK_DPI_DIV8:
		sci_glb_write(REG_ANLG_PHY_G1_ANALOG_MIPI_DSI_4LANE_DSI_ISO_SW, 0x0, 0xFFFF);
		sci_glb_write(REG_AP_CLK_CORE_CGM_DISPC0_DPI_CFG, BIT(2), BIT(0) | BIT(1) | BIT(2));
		break;
	default:
