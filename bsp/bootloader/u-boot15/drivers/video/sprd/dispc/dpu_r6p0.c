...
static void dpu_config_dsc_param(struct dispc_context *ctx)
{
	struct dpu_reg *reg = (struct dpu_reg *)ctx->base;
	...
	if (panel->dual_dsi_en)
		...
		reg->dsc_pic_size = (panel->height << 16) |
			(panel->width << 0);
	if (panel->dual_dsi_en)
		reg->dsc_h_timing = ((rgb->hsync >> 1) << 0) |
			((rgb->hbp >> 1) << 8) |
			((rgb->hfp >> 1) << 20);
	else
		reg->dsc_h_timing = (rgb->hsync << 0) |
			(rgb->hbp << 8) |
			(rgb->hfp << 20);
	reg->dsc_v_timing = (rgb->vsync << 0) |
		(rgb->vbp << 8) |
		(rgb->vfp << 20);
	...
	switch (panel->dsc_mode) {
		...
		case DSC_720_2560_720_8:
			...
			if (panel->dual_dsi_en) {
				reg->dsc1_pic_size = (panel->height << 16) |
					((panel->width >> 1)  << 0);
				reg->dsc1_h_timing = ((rgb->hsync >> 1) << 0) |
					((rgb->hbp >> 1) << 8) |
					((rgb->hfp >> 1) << 20);
				reg->dsc1_v_timing = (rgb->vsync << 0) |
					(rgb->vbp << 8) |
					(rgb->vfp << 20);
...
static int dpu_init(struct dispc_context *ctx)
{
	...
	if ((panel->work_mode == SPRD_MIPI_MODE_CMD) && panel->dsc_en) {
		reg->dpu_cfg0 |= BIT(1);
		ctx->is_single_run = true;
	}

	reg->dpu_cfg1 = 0xc4caca;
	dpu_clean(ctx);
		    ...
		    struct sprd_adf_hwlayer *hwlayer)
{
	...
	reg->layer_enable |= (1 << hwlayer->hwlayer_id);
