...
static int dpu_write_back_config(struct dispc_context *ctx)
{
	...
	wb_hdr_size = XFBC8888_HEADER_SIZE(panel->width, panel->height);
	...
	region[0].pos_x = 0;
	region[0].pos_y = 0;
	region[0].size_w = panel->width;
	region[0].size_h = panel->height;
	...
	wb_layer.pitch[0] = panel->width * 4;
	wb_layer.iova_plane[0] = (!wb_xfbc_en) ? wb_addr_v :
				wb_addr_v + wb_hdr_size;
			...
			struct sprd_restruct_config *config)
{
	...
	if (ctx->if_type == SPRD_DISPC_IF_EDPI)
		/* make sure the dpu is in stop status, because sharkl5 edpi no shadow regs */
				 ...
				 u32 count)
{
	...
	for (i = 0; i < count; i++) {
		...
		case ENHANCE_CFG_ID_MODULE_EN:
			mod = cfg[i].param;
			reg->dpu_enhance_cfg = (mod->cabc_en << 8) |
						(mod->slp_mask_en << 7) |
						(mod->ltm_en << 6) |
						(mod->gamma_en << 5) |
						(mod->slp_en << 4) |
