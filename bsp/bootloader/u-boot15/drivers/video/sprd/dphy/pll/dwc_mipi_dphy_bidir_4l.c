...
static int dphy_pll_config(struct dphy_context *ctx)
{
	...
	sprd_regmap_write(regmap, 0x44, data);

	/* setup PLL */
	/* vco range  [7]|[6:3]|[2:1]|[0] */
	data = (1 << 7) | (ranges[j].vco_range << 3) | (0 << 1) | 0;
//	sprd_regmap_write(regmap, 0x10, data);
...
//	sprd_regmap_write(regmap, 0x07, 0x04);
	...
	sprd_regmap_write(regmap, 0x18, data);
...
//	sprd_regmap_write(regmap, 0x60, 0x42);
	/* prepare time */
//	sprd_regmap_write(regmap, 0x61, 0x0);
	/* zero time */
//	sprd_regmap_write(regmap, 0x62, 0x0);
	/* data lane lpx, about 52ns */
//	sprd_regmap_write(regmap, 0x70, 0x42);
	/* prepare time */
//	sprd_regmap_write(regmap, 0x71, 0x0);
	/* zero time */
//	sprd_regmap_write(regmap, 0x72, 0x0);
