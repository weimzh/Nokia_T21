...
static int dphy_set_pll_reg(struct regmap *regmap, struct dphy_pll *pll, bool dsi_switch)
{
	if (!pll || !pll->fvco)
		goto FAIL;
	...
	sprd_regmap_write(regmap, 0x03, regs._03.val);
	sprd_regmap_write(regmap, 0x04, regs._04.val);
	sprd_regmap_write(regmap, 0x07, regs._07.val);
	sprd_regmap_write(regmap, 0x08, regs._08.val);
	sprd_regmap_write(regmap, 0x09, regs._09.val);
	sprd_regmap_write(regmap, 0x0a, regs._0a.val);
	sprd_regmap_write(regmap, 0x0b, regs._0b.val);
	sprd_regmap_write(regmap, 0x0c, regs._0c.val);
	sprd_regmap_write(regmap, 0x0d, regs._0d.val);
	sprd_regmap_write(regmap, 0x0e, regs._0e.val);
	sprd_regmap_write(regmap, 0x0f, regs._0f.val);
	sprd_regmap_write(regmap, 0x06, regs._06.val);
#ifdef CONFIG_SOC_QOGIRN6PRO
	if (dsi_switch) {
		sprd_regmap_write(regmap, 0xf1, 0x27);
		sprd_regmap_write(regmap, 0x4d, 0x60);
		sprd_regmap_write(regmap, 0x7d, 0x00);
	}
#endif
...
static int dphy_pll_config(struct dphy_context *ctx)
{
	...
	if (ret)
		goto FAIL;
	ret = dphy_set_pll_reg(regmap, &pll, ctx->is_dsi_switch);
	if (ret)
		goto FAIL;

#ifdef CONFIG_MIPI_DRIVE_CAPABILITY
	regs._25.bits.ldoop4_sel = CONFIG_MIPI_DRIVE_CAPABILITY;
...
static int dphy_set_timing_regs(struct regmap *regmap, int type, u8 val[])
{
	switch (type) {
	case REQUEST_TIME:
		sprd_regmap_write(regmap, 0x31, val[CLK]);
		sprd_regmap_write(regmap, 0x41, val[DATA]);
		sprd_regmap_write(regmap, 0x51, val[DATA]);
		sprd_regmap_write(regmap, 0x61, val[DATA]);
		sprd_regmap_write(regmap, 0x71, val[DATA]);

		sprd_regmap_write(regmap, 0x90, val[CLK]);
		sprd_regmap_write(regmap, 0xa0, val[DATA]);
		sprd_regmap_write(regmap, 0xb0, val[DATA]);
		sprd_regmap_write(regmap, 0xc0, val[DATA]);
		sprd_regmap_write(regmap, 0xd0, val[DATA]);
		break;
	case PREPARE_TIME:
		sprd_regmap_write(regmap, 0x32, val[CLK]);
		sprd_regmap_write(regmap, 0x42, val[DATA]);
		sprd_regmap_write(regmap, 0x52, val[DATA]);
		sprd_regmap_write(regmap, 0x62, val[DATA]);
		sprd_regmap_write(regmap, 0x72, val[DATA]);

		sprd_regmap_write(regmap, 0x91, val[CLK]);
		sprd_regmap_write(regmap, 0xa1, val[DATA]);
		sprd_regmap_write(regmap, 0xb1, val[DATA]);
		sprd_regmap_write(regmap, 0xc1, val[DATA]);
		sprd_regmap_write(regmap, 0xd1, val[DATA]);
		break;
	case ZERO_TIME:
		sprd_regmap_write(regmap, 0x33, val[CLK]);
		sprd_regmap_write(regmap, 0x43, val[DATA]);
		sprd_regmap_write(regmap, 0x53, val[DATA]);
		sprd_regmap_write(regmap, 0x63, val[DATA]);
		sprd_regmap_write(regmap, 0x73, val[DATA]);

		sprd_regmap_write(regmap, 0x92, val[CLK]);
		sprd_regmap_write(regmap, 0xa2, val[DATA]);
		sprd_regmap_write(regmap, 0xb2, val[DATA]);
		sprd_regmap_write(regmap, 0xc2, val[DATA]);
		sprd_regmap_write(regmap, 0xd2, val[DATA]);
		break;
	case TRAIL_TIME:
		sprd_regmap_write(regmap, 0x34, val[CLK]);
		sprd_regmap_write(regmap, 0x44, val[DATA]);
		sprd_regmap_write(regmap, 0x54, val[DATA]);
		sprd_regmap_write(regmap, 0x64, val[DATA]);
		sprd_regmap_write(regmap, 0x74, val[DATA]);

		sprd_regmap_write(regmap, 0x93, val[CLK]);
		sprd_regmap_write(regmap, 0xa3, val[DATA]);
		sprd_regmap_write(regmap, 0xb3, val[DATA]);
		sprd_regmap_write(regmap, 0xc3, val[DATA]);
		sprd_regmap_write(regmap, 0xd3, val[DATA]);
		break;
	case EXIT_TIME:
		sprd_regmap_write(regmap, 0x36, val[CLK]);
		sprd_regmap_write(regmap, 0x46, val[DATA]);
		sprd_regmap_write(regmap, 0x56, val[DATA]);
		sprd_regmap_write(regmap, 0x66, val[DATA]);
		sprd_regmap_write(regmap, 0x76, val[DATA]);

		sprd_regmap_write(regmap, 0x95, val[CLK]);
		sprd_regmap_write(regmap, 0xA5, val[DATA]);
		sprd_regmap_write(regmap, 0xB5, val[DATA]);
		sprd_regmap_write(regmap, 0xc5, val[DATA]);
		sprd_regmap_write(regmap, 0xd5, val[DATA]);
		break;
	case CLKPOST_TIME:
		sprd_regmap_write(regmap, 0x35, val[CLK]);
		sprd_regmap_write(regmap, 0x94, val[CLK]);
		break;
...
static void dphy_force_pll(struct dphy_context *ctx, int force)
{
	...
	sprd_regmap_write(regmap, 0x0e, data);
}
