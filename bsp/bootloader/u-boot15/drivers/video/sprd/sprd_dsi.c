...
struct sprd_dsi dsi_slave_device;
...
__weak struct dsi_glb_ops *dsi_s_glb_ops_attach(void)
{
	return NULL;
}
...
static int32_t dsi_dpi_video(struct sprd_dsi *dsi)
{
	...
	if (panel->dual_dsi_en) {
		param.hact = panel->width >> panel->bv3_en >> 1;
		param.hsync = timing->hsync >> 1;
		param.hbp = timing->hbp >> 1;
		param.hline = (panel->width
				+ timing->hsync + timing->hbp + timing->hfp) >> 1;
	} else {
...
int sprd_dsi_slave_probe(void)
{
	struct sprd_dsi *dsi_s;

	dsi_s = &dsi_slave_device;
	dsi_s->panel = panel_info_attach();
	dsi_s->core = dsi_core_ops_attach();
	dsi_s->glb = dsi_s_glb_ops_attach();

	dsi_context_init(dsi_s);
	sprd_dsi_resume(dsi_s);
