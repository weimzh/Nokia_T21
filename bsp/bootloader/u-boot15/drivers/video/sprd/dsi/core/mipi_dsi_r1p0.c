...
static bool dsi_check_version(struct dsi_context *ctx)
{
	...
	if (read32(&reg->DSI_VERSION) == 0x100)
		return true;
	else if (read32(&reg->DSI_VERSION) == 0x200)
		return true;
	else if (read32(&reg->DSI_VERSION) == 0x300)
		return true;
	else if (read32(&reg->DSI_VERSION) == 0x301)
		return true;
	else
		return false;
}
...
static struct dsi_core_ops mipi_dsi_r1p0_ops = {
	...
	.dpi_color_mode_pol             = NULL,
	.dpi_shut_down_pol              = NULL,
	.dpi_hsync_pol                  = NULL,
	.dpi_vsync_pol                  = NULL,
	.dpi_data_en_pol                = NULL,
	...
	.dpi_null_packet_en             = NULL,
	.dpi_multi_packet_en            = NULL,
...
/*
 * In fact, there is no need to define this new structure for
 * match_table2. Just use mipi_dsi_r1p0_ops structure is a simple
 * way. Because the only deference between mipi-dsi-r1p0 and
 * dsi-ctrl-r1p0 IP is just the gen_cmd_rdcmd_done bit of
 * CMD_MODE_STATUS. But what the fuck thing is that the IP version
 * of mipi-dsi-r1p0 and dsi-ctrl-r1p0 are the same! That use IP
 * version to distinguish the implementation of dsi_is_bta_returned()
 * can not be possible. So I copy and restruct the dsi_ctrl_r1p0_ops.
 */
static struct dsi_core_ops dsi_ctrl_r1p0_ops = {
	.check_version                  = dsi_check_version,
	.power_en                       = dsi_power_enable,
	.video_mode                     = dsi_video_mode,
	.cmd_mode                       = dsi_cmd_mode,
