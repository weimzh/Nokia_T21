...
void start_watchdog(uint64_t init_time_ms)
{
    wdt_config_t config;
    //clk on
    ...
    config.val      = init_time_ms <= WDG_REBOOT_MS ? 0x50 : ((init_time_ms / 1000) * WDG_CLK + ((init_time_ms % 1000) * WDG_CLK) / 1000);
...
void stop_watchdog(void)
{
    ...
    printf("Watch Dog Trace: WDG_TimerStop\n");
    wdt_int_clr();
