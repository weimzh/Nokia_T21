...
#ifndef SPRD_GPIO_BANK_NR
#define SPRD_GPIO_BANK_NR	16
#endif
#define SPRD_GPIO_BANK_MASK    0xffff//GENMASK(15, 0)
#ifndef SPRD_GPIO_BIT
...
struct gpio_irq {
	u16          irq;
	u16          ngpio;
	GPIO_CALLBACK  int_table[NR_D_DIE_GPIOS];
}sprd_gpio_cfg = {0};
#endif
...
void sprd_gpio_int_reg(unsigned int id_num, GPIO_CALLBACK gpio_callback)
{
	if(id_num >= NR_D_DIE_GPIOS) {
		WARN(1, "gpio number is too larger:%d\r\n", id_num);
			   return;
	}

	if(sprd_gpio_cfg.int_table[id_num] == NULL)
		sprd_gpio_cfg.int_table[id_num] = gpio_callback;
	   else
		WARN(1, "GPIO_%d callback function is configured repeatedly\r\n", id_num);
}

static void sprd_gpio_reg_update(struct gpio_chip *chip,
			     u32 offset, u16 reg, int value)
{
	struct gpio_info gpio_info;
	u32 tmp;

	__get_gpio_base_info (offset, &gpio_info);

	tmp = gpio_reg_get(gpio_info.base_addr + reg, gpio_info.die);
	...
	gpio_reg_set(gpio_info.base_addr + reg, gpio_info.die, tmp);
}

int sprd_gpio_get_mis(struct gpio_chip *chip, unsigned offset)
{
	unsigned gpio_id = offset;
	struct gpio_info gpio_info;
	...
	return !!(gpio_reg_get(gpio_info.base_addr + GPIO_MIS, gpio_info.die)
	                     & BIT(SPRD_GPIO_BIT(offset)));
}

void sprd_gpio_irq_mask(struct gpio_chip *chip, unsigned offset)
{
	sprd_gpio_reg_update(chip, offset, GPIO_IE, 0);
}

void sprd_gpio_irq_ack(struct gpio_chip *chip, unsigned offset)
{
	sprd_gpio_reg_update(chip, offset, GPIO_IC, 1);
}

void sprd_gpio_irq_unmask(struct gpio_chip *chip, unsigned offset)
{
	sprd_gpio_reg_update(chip, offset, GPIO_IC, 1);
	sprd_gpio_reg_update(chip, offset, GPIO_IE, 1);
}

int sprd_gpio_irq_set_type(struct gpio_chip *chip,
				  unsigned int flow_type, u32 offset)
{
	switch (flow_type) {
	case IRQ_TYPE_EDGE_RISING:
		sprd_gpio_reg_update(chip, offset, GPIO_IS, 0);
		sprd_gpio_reg_update(chip, offset, GPIO_IBE, 0);
		sprd_gpio_reg_update(chip, offset, GPIO_IEV, 1);
		sprd_gpio_reg_update(chip, offset, GPIO_IC, 1);
		break;
	case IRQ_TYPE_EDGE_FALLING:
		sprd_gpio_reg_update(chip, offset, GPIO_IS, 0);
		sprd_gpio_reg_update(chip, offset, GPIO_IBE, 0);
		sprd_gpio_reg_update(chip, offset, GPIO_IEV, 0);
		sprd_gpio_reg_update(chip, offset, GPIO_IC, 1);
		break;
	case IRQ_TYPE_EDGE_BOTH:
		sprd_gpio_reg_update(chip, offset, GPIO_IS, 0);
		sprd_gpio_reg_update(chip, offset, GPIO_IBE, 1);
		sprd_gpio_reg_update(chip, offset, GPIO_IC, 1);
		break;
	case IRQ_TYPE_LEVEL_HIGH:
		sprd_gpio_reg_update(chip, offset, GPIO_IS, 1);
		sprd_gpio_reg_update(chip, offset, GPIO_IBE, 0);
		sprd_gpio_reg_update(chip, offset, GPIO_IEV, 1);
		break;
...
static int sprd_gpio_irq_handler(void)
{
	unsigned int bank, n, gpio, state;
		u32 reg, base;

	for (bank = 0; bank * SPRD_GPIO_BANK_NR < NR_D_DIE_GPIOS; bank++) {
		base = __get_base_addr (bank * SPRD_GPIO_BANK_NR);
		reg = gpio_reg_get(base + GPIO_MIS, D_DIE) &
			SPRD_GPIO_BANK_MASK;
		while (reg) {
		n = __ffs(reg);
			reg &= ~(1 << n);
			gpio = bank * SPRD_GPIO_BANK_NR + n;
			sprd_gpio_irq_ack(NULL, gpio);
			sprd_gpio_irq_mask(NULL, gpio);
			state = sprd_gpio_get(NULL, gpio);
			if (sprd_gpio_cfg.int_table[gpio] != NULL)
				(*(sprd_gpio_cfg.int_table[gpio]))(gpio, state);
			else
				printf("gpio:%d isr callback is null!", gpio);
			sprd_gpio_irq_unmask(NULL, gpio);
		}
	}
	return 0;
}

PUBLIC u32 GPIO_AddCallbackToIntTable(
		u32 gpio_id,
		GPIO_CALLBACK gpio_callback_fun)
{
	sprd_gpio_int_reg(gpio_id, gpio_callback_fun);
	GPIO_EnableIntCtl(gpio_id);
	return 0;
}

PUBLIC u32 GPIO_DeleteFromIntTable (u32 gpio_id){
...
void GPIO_Disable (unsigned int gpio_id)
{
	sprd_gpio_free(NULL, gpio_id);
}

void GPIO_EnableInt (void)
{
	gic_interrupt_enable(sprd_gpio_cfg.irq, 1);
}

void GPIO_DisableInt (void)
{
	gic_interrupt_enable(sprd_gpio_cfg.irq, 0);
}

void GPIO_EnableIntCtl(u32 gpio_id)
{
	sprd_gpio_irq_unmask(NULL, gpio_id);
}

void GPIO_DisableIntCtl(u32 gpio_id)
{
	sprd_gpio_irq_mask(NULL, gpio_id);
}

void GPIO_SetDirection (unsigned int  gpio_id, int directions)
{
	if (directions)
		sprd_gpio_direction_output(NULL, gpio_id, 0);
	else
		sprd_gpio_direction_input(NULL, gpio_id);

}

void GPIO_SetInterruptsSense (unsigned int gpio_id, int sensetype)
{
	sprd_gpio_irq_set_type(NULL, sensetype, gpio_id);
}

void GPIO_ClearIntStatus (unsigned int gpio_id)
{
	sprd_gpio_irq_ack(NULL, gpio_id);
}
#endif
...
#endif
#ifdef CONFIG_SPRD_GPIO_IRQ
	sprd_gpio_cfg.irq = TB_GPIO_INT;
	irq_handler_register(sprd_gpio_cfg.irq, sprd_gpio_irq_handler);
	GPIO_EnableInt();
#endif
}
