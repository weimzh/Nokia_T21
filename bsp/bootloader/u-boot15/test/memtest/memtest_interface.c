...
int compare_regions(ulv *bufa, ulv *bufb, size_t count)
{
	...
	for (i = 0; i < count; i++, p1++, p2++) {
		if (*p1 != *p2) {
			...
			fprintf( "FAILURE: 0x%lx != 0x%lx at physical address 0x%lx and 0x%lx.\n",(ul) *p1, (ul) *p2, (ul) p1, (ul) p2);
			//while(1);
		}
	}
	return r;
...
int test_stuck_address(ulv *bufa, ulv *bufb, size_t count) {
	...
	for (j = 0; j < 16; j++) {
		...
		for (i = 0; i < count; i++, p1++) {
			...
			{
				//physaddr = sr_addr + (i * sizeof(ul));
				fprintf("FAILURE: possible bad address line at physical address 0x%lx.\n", (ul) p1);
				//while(1);
			}
		}
	}
...
#           else
			...
			fprintf("Compare times: %d.\n", (j+1));
			fprintf("FAILURE: 0x%lx != 0x%lx at physical address 0x%lx.\n", (ul) *p1, (ul) *p2, physaddr);
                } else {
			fprintf("Compare times: %d.\n", (j+1));
			fprintf("FAILURE: 0x%lx != 0x%lx at offset 0x%lx.\n", (ul) *p1, (ul) *p2, (ul) (i * sizeof(ul)));
                }
...
void ddr_mem_test(ulong dram_base, ulong test_len)
{
	ul length, halflen, count;
	int regval;

	halflen = test_len / 2;
	sr_addr = (ulv *)dram_base;
	dst_addr = (ulv *) (dram_base + halflen);
	count = halflen / sizeof(ul);

	fprintf("dram_base 0x%lx, source address 0x%lx, dest address 0x%lx,test length 0x%lx\n",
		dram_base, (ul) sr_addr, (ul) dst_addr, test_len);
