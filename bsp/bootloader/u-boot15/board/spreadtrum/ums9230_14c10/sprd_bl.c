...
void set_backlight(uint32_t brightness)
{
	...
	__raw_bits_or((0x1 << 0), REG_AON_CLK_CGM_PWM0_CFG + index * 4);//ext_26m select
	__raw_bits_or((BIT_AON_APB_PWM0_EB << PWM_INDEX), REG_AON_APB_APB_EB2); //PWMx EN
	...
	mipi_dsi_gen_write(dsi,&set_bl_seq[0], 2);
#endif
#endif
	return;
}