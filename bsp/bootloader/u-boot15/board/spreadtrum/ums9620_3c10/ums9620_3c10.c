...
#define BOARDID_GPIO_0		133
#define BOARDID_GPIO_1		134
#define BOARDID_GPIO_2		135
#define BOARDID_GPIO_3		132
#define BOARDID_GPIO_4		131
...
int board_init(void)
{
	...
	thm_overheate_en();
	usb_eye_pattern_set();
	if(sprd_get_pcbversion() == 0xb || sprd_get_pcbversion() == 0xf) {
...
int sprd_get_bandinfo(void)
{
    ...
    pcbversion = sprd_get_pcbversion();

    if(pcbversion == 0 || pcbversion == 1 || pcbversion == 7 || pcbversion == 0xf)
        return 0; //hw_ver00
    else if(pcbversion == 3 || pcbversion == 0xb)
        return 1; //hw_ver01
    else
        return 0;
...
int sprd_get_pcbversion(void)
{
	...
	gpio_val[0] = gpio_state(BOARDID_GPIO_0);
	gpio_val[1] = gpio_state(BOARDID_GPIO_1);
	gpio_val[2] = gpio_state(BOARDID_GPIO_2);
	gpio_val[3] = gpio_state(BOARDID_GPIO_3);
	gpio_val[4] = gpio_state(BOARDID_GPIO_4);

	for(i = 0; i < 5; i++) {
		boardid |= (gpio_val[i] << i);
	}

	debugf("pcbversion is %d\n",boardid);
